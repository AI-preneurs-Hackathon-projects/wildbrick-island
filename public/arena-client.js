...
export function createArenaClient({onSnapshot=()=>{},onStatus=()=>{},onEvent=()=>{},onError=()=>{}}={},runtime={}){
 ...
 async function request(path,body){const started=diagnostics?clock():0;let requestStatus=0,failure=0;if(diagnostics)diagnose('request-start',{});const abort=new AbortController(),timeout=setTimer(()=>abort.abort(new DOMException('Arena request timed out','TimeoutError')),12000);try{const r=await fetcher(path,{keepalive:path.endsWith('/leave'),method:body?'POST':'GET',headers:body?{'Content-Type':'application/json'}:undefined,body:body?JSON.stringify(body):undefined,signal:abort.signal,cache:'no-store'});requestStatus=r.status;let d;try{d=await r.json();}catch{failure=4;throw Object.assign(Error('The Arena server returned an invalid response. Try joining again.'),{status:r.status});}if(!r.ok){failure=5;const e=Error(d.error||'The arena could not connect.');e.status=r.status;throw e;}return d;}catch(e){failure=abort.signal.aborted?2:e.name==='AbortError'?3:failure||1;if(abort.signal.aborted)throw Object.assign(Error('The Arena connection timed out. Please try Join Arena again; your retry will reuse the same player.'),{code:'timeout'});if(e.name==='AbortError')throw Object.assign(Error('The Arena connection was interrupted. Please try Join Arena again.'),{code:'canceled'});throw e;}finally{clearTimer(timeout);if(diagnostics)diagnose('request-end',{at:clock(),duration:clock()-started,status:requestStatus,failure});}}
 ...
 function accept(s){
  ...
  const previous=diagnostics&&self?{x:self.x,y:self.y,z:self.z,ack:snapshot?.players.find(p=>p.id===snapshot.self)?.motion.frame}:null,previousSuccess=lastSuccess;
  ...
  if(diagnostics)diagnose('snapshot',{gap:previous?clock()-previousSuccess:undefined,ackAdvance:!teleport&&previous?authoritative.motion.frame-previous.ack:undefined,correction:!teleport&&previous?Math.hypot(self.x-previous.x,self.y-previous.y,self.z-previous.z):undefined});
 ...
 function tick(dt,newInput){
  ...
  const predictionReason=diagnostics?(!self?0:!credentials?5:snapshot?.round?.status==='finished'?4:!fresh?2:frames.length>=MAX_MOVE_FRAMES?3:1):0;
  ...
  }else{accumulator=0;jumpQueued=false;if(self){self.speed=0;self.vertical=0;}if(credentials&&clock()-lastSuccess>2500)status('reconnecting');}
  ...
  const pose=self?view.update(self,fresh?dt:0,{correct:!!moving&&!!fresh}):null;if(pose){pose.aimPitch=weaponAim(pose,input.weaponPitch).pitch;}
  if(diagnostics)diagnose('tick',{at:clock(),age:snapshot?clock()-lastSuccess:undefined,pending:frames.length,reason:predictionReason});
  return pose;
 }
