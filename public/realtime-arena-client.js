...
export function createRealtimeArenaClient({onSnapshot=()=>{},onStatus=()=>{},onEvent=()=>{},onError=()=>{}}={},runtime={}){
  ...
  return await new Promise((resolve,reject)=>{
   ...
   ws.onmessage=event=>{let message;try{message=JSON.parse(event.data);}catch{return fail(Error('The Arena sent an invalid realtime message.'));}
    try{
     ...
     if(message.type==='joined'&&!joined){joined=true;clearTimer(guard);credentials={session:message.session,token:message.token};joinTicket=credentials;accept(message.snapshot,{full:true});scheduleSend(0);resolve(true);return;}
 ...
 async function join(name,room,avatarColor,create=false,ticket){
  if(joining)return false;joining=true;lifecycle++;closed=false;connected=false;clearScheduled();rejectRequests(Error('Arena join replaced.'));roomCode=String(room||'').trim().toUpperCase();joinArgs={name,avatarColor,create};joinTicket=credentials||joinTicket||{session:crypto.randomUUID(),token:crypto.randomUUID().replaceAll('-','')+crypto.randomUUID().replaceAll('-','')};credentials=null;snapshot=null;self=null;revision=-1;lastEvent=0;seq=0;commandId=0;commands=[];pendingInputs=[];input={};accumulator=0;motionEpoch=0;view.reset();onStatus('joining');
  lastPacketAt=0;previewAt=-Infinity;try{return await connect(false,ticket);}catch(error){closed=true;connected=false;if([401,410].includes(error.status))joinTicket=null;try{socket?.close();}catch{}socket=null;onStatus('offline');onError(error.message,error.status);return false;}finally{joining=false;}
 }
 ...
 async function leave(){lifecycle++;closed=true;joining=false;clearScheduled();const ws=socket;try{if(connected)await ask('leave');}catch{}finally{connected=false;credentials=null;joinTicket=null;joinArgs=null;socket=null;try{ws?.close(1000,'left');}catch{}rejectRequests(Error('Arena left.'));snapshot=null;self=null;commands=[];pendingInputs=[];input={};lastPacketAt=0;previewAt=-Infinity;view.reset();onStatus('offline');}}
 ...
 return {join,start,leave,tick,command,getTicket,blueprints,get room(){return roomCode;},get active(){return !!snapshot&&!!self&&!closed;},get connected(){return connected;},get self(){return view.state||self;},get snapshot(){return snapshot;},get stale(){return !connected||clock()-lastSuccess>1000;},serverTime};
}
