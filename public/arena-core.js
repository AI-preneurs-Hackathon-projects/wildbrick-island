...
function newRound(id,time,status='active'){return {id,status,startsAt:time,endsAt:time+ROUND_MS,results:[],intermissionEndsAt:null};}
function ensureRound(room){if(!room.round)room.round=newRound(1,room.time);const r=room.round;delete r.scores;delete r.previousResults;if(r.status==='finished'&&!r.intermissionEndsAt)r.intermissionEndsAt=r.endsAt+INTERMISSION_MS;for(const p of Object.values(room.players))if(p.roundId===undefined)p.roundId=r.id;return r;}
...
function finishRound(room){const round=ensureRound(room);if(round.status!=='active')return;round.results=roundScores(room);round.status='finished';round.intermissionEndsAt=round.endsAt+INTERMISSION_MS;room.projectiles=[];for(const p of Object.values(room.players)){p.input={};p.melee=null;p.building=null;p.speed=0;p.vertical=0;}event(room,'round-ended',{round:round.id});}
function restartRound(room,id){const old=ensureRound(room);if(old.status!=='finished'||old.id!==id)return;room.round=newRound(old.id+1,room.time,Object.keys(room.players).length?'active':'waiting');room.projectiles=[];room.drops=[];room.nextDrop=room.time+4000;
 for(const p of Object.values(room.players)){const kit=p.kit;if(p.health<=0)respawn(room,p);Object.assign(p,{roundId:room.round.id,kit,kills:0,deaths:0,health:100,mountHealth:kit.stats.mountMax,respawnAt:0,protectedUntil:room.time+3000,defenseUntil:0,speedUntil:0,heat:0,overheatedUntil:0,nextShot:0,buildReadyAt:room.time,building:null,melee:null,input:{},inputAt:0,speed:0,vertical:0,vy:0,grounded:false,jumpRemaining:0,aimPitch:0});p.spawnSerial=(p.spawnSerial||0)+1;if(p.motion)p.motion=newMotion(room.time);}
...
export function removePlayer(room,id){if(room.round)room.round.results=room.round.results.filter(p=>p.id!==id);if(room.players[id]){event(room,'leave',{player:id,name:room.players[id].name});delete room.players[id];}}
...
export function advanceRoom(room,now){ensureRound(room);now=Math.max(room.time,now);
 ...
 while(room.time<now){const round=room.round;
  ...
  if(round.status==='finished'){const boundary=round.intermissionEndsAt;if(now<boundary){room.time=now;break;}room.time=Math.max(room.time,boundary);restartRound(room,round.id);continue;}
 ...
 if(room.round.status==='finished'&&now>=room.round.intermissionEndsAt){room.time=room.round.intermissionEndsAt;restartRound(room,room.round.id);room.time=now;}
 return room;}
