...
export const DEFAULT_ARENA="ISLAND",RESPAWN_MS=12000,INPUT_STALE_MS=750,ROUND_MS=5*60*1000;
...
function newRound(id,time,status='active',previousResults=[]){return {id,status,startsAt:time,endsAt:time+ROUND_MS,results:[],scores:{},previousResults};}
function ensureRound(room){if(!room.round)room.round=newRound(1,room.time);return room.round;}
function roundScores(room){const scores={...ensureRound(room).scores};for(const p of Object.values(room.players))scores[p.id]={id:p.id,name:p.name,kills:p.kills,deaths:p.deaths};return Object.values(scores).sort((a,b)=>b.kills-a.kills||a.deaths-b.deaths||a.name.localeCompare(b.name)||a.id.localeCompare(b.id));}
function finishRound(room){const round=ensureRound(room);if(round.status!=='active')return;round.results=roundScores(room);round.status='finished';room.projectiles=[];for(const p of Object.values(room.players)){p.input={};p.melee=null;p.building=null;p.speed=0;p.vertical=0;}event(room,'round-ended',{round:round.id});}
function restartRound(room,id){const old=ensureRound(room);if(old.status!=='finished'||old.id!==id)return;room.round=newRound(old.id+1,room.time,'active',old.results);room.projectiles=[];room.drops=[];room.nextDrop=room.time+4000;
 for(const p of Object.values(room.players)){const kit=p.kit;if(p.health<=0)respawn(room,p);Object.assign(p,{kit,kills:0,deaths:0,health:100,mountHealth:kit.stats.mountMax,respawnAt:0,protectedUntil:room.time+3000,defenseUntil:0,speedUntil:0,heat:0,overheatedUntil:0,nextShot:0,buildReadyAt:room.time,building:null,melee:null,input:{},inputAt:0,speed:0,vertical:0,jumpRemaining:0,aimPitch:0});p.spawnSerial=(p.spawnSerial||0)+1;if(p.motion)p.motion=newMotion(room.time);}
 event(room,'round-started',{round:room.round.id});}
export function newRoom(now=Date.now(),epoch='local'){return {round:newRound(1,now,'waiting'),epoch,revision:0,time:now,players:{},projectiles:[],drops:[],damage:{},destroyed:{},placed:[],events:[],eventId:0,nextDrop:now+4000,dropIndex:0,seed:734159};}
...
export function advanceRoom(room,now){const round=ensureRound(room);now=Math.max(room.time,now);if(round.status==='active'){
 ...
 }room.time=now;return room;}
export function applyInput(room,id,packet,now,kit=null){const p=room.players[id];if(!p)throw Object.assign(Error('Your arena session expired. Join again.'),{status:410});p.lastSeen=now;if(!Number.isSafeInteger(packet.seq)||packet.seq<=p.lastSeq)return;p.lastSeq=packet.seq;if(p.motion&&Number.isSafeInteger(packet.motionEpoch)&&packet.motionEpoch!==(p.spawnSerial||0)){if(Number.isSafeInteger(packet.command?.id))p.lastCommand=Math.max(p.lastCommand,packet.command.id);return;}const round=ensureRound(room);if(packet.roundId!==undefined&&packet.roundId!==round.id){if(Number.isSafeInteger(packet.command?.id))p.lastCommand=Math.max(p.lastCommand,packet.command.id);return;}if(round.status==='finished'){const c=packet.command;if(c&&Number.isSafeInteger(c.id)&&c.id>p.lastCommand){p.lastCommand=c.id;if(c.type==='restart')restartRound(room,c.roundId);}return;}p.input=cleanInput(packet.input);p.aimPitch=weaponAim(p,p.input.weaponPitch).pitch;p.inputAt=now;applyFrames(room,p,packet,now);
