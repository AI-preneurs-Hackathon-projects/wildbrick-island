...
function ensureRound(room){if(!room.round)room.round=newRound(1,room.time);const r=room.round;if(!MAP_CYCLE.includes(r.mapId))r.mapId='island';delete r.scores;delete r.previousResults;if(r.status==='finished'&&!r.intermissionEndsAt)r.intermissionEndsAt=r.endsAt+INTERMISSION_MS;for(const p of Object.values(room.players))if(p.roundId===undefined)p.roundId=r.id;return r;}
...
function restartRound(room,id){const old=ensureRound(room);if(old.status!=='finished'||old.id!==id)return;room.round=newRound(old.id+1,room.time,Object.keys(room.players).length?'active':'waiting',nextMap(old.mapId));room.projectiles=[];room.drops=[];room.placed=[];room.damage={};room.destroyed={};room.events=[];room.nextDrop=room.time+4000;room.dropIndex=0;
 for(const p of Object.values(room.players)){const kit=p.kit;const [x,z]=safeSpawn(room,p.id,kit);Object.assign(p,{x,y:0,z,yaw:Math.PI,roundId:room.round.id,kit,kills:0,deaths:0,health:100,mountHealth:kit.stats.mountMax,respawnAt:0,protectedUntil:room.time+3000,defenseUntil:0,speedUntil:0,heat:0,overheatedUntil:0,nextShot:0,buildReadyAt:room.time,building:null,melee:null,input:{},inputAt:0,speed:0,vertical:0,vy:0,grounded:kit.stats.movement!=='fly',jumpRemaining:0,aimPitch:0});p.spawnSerial=(p.spawnSerial||0)+1;if(p.motion)p.motion=newMotion(room.time);}
