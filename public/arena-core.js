...
export const DEFAULT_ARENA="ISLAND",RESPAWN_MS=12000,INPUT_STALE_MS=750,LOBBY_MS=15000,ROUND_MS=5*60*1000,INTERMISSION_MS=15000;
...
function newRound(id,time,status='active',mapId=mapForRound(id)){return {id,mapId,status,startsAt:status==='active'?time:null,endsAt:status==='active'?time+ROUND_MS:null,lobbyEndsAt:null,results:[],intermissionEndsAt:null};}
...
export function newRoom(now=Date.now(),epoch='local'){return {match:newMatch(),round:newRound(1,now,'waiting'),epoch,revision:0,time:now,players:{},projectiles:[],drops:[],items:[],damage:{},destroyed:{},placed:[],events:[],eventId:0,nextDrop:now+4000,dropIndex:0,seed:734159};}
...
// use queuePlayer so the server owns the shared lobby deadline.
...
export function queuePlayer(room,id,name,now=room.time){ensureRound(room);resetCompletedMatch(room,now);const round=ensureRound(room);if(round.status!=='waiting')throw Object.assign(Error('This match is already in progress. Join after the current game ends.'),{status:409});const p=createPlayer(room,id,name,now);if(!round.lobbyEndsAt){round.lobbyEndsAt=now+LOBBY_MS;event(room,'lobby-started',{endsAt:round.lobbyEndsAt});}return p;}
export function removePlayer(room,id){if(room.players[id]){event(room,'leave',{player:id,name:room.players[id].name});delete room.players[id];if(room.round?.status==='waiting'&&!Object.keys(room.players).length)room.round.lobbyEndsAt=null;}}
...
export function advanceRoom(room,now){ensureRound(room);now=Math.max(room.time,now);
 ...
 while(room.time<now){const round=room.round;
  if(round.status==='waiting'){const boundary=round.lobbyEndsAt;if(!boundary||now<boundary){room.time=now;break;}room.time=Math.max(room.time,boundary);round.status='active';round.startsAt=room.time;round.endsAt=room.time+ROUND_MS;for(const p of Object.values(room.players)){p.roundId=round.id;p.input={};p.inputAt=0;p.protectedUntil=room.time+3000;p.buildReadyAt=room.time;p.spawnSerial=(p.spawnSerial||0)+1;if(p.motion)p.motion=newMotion(room.time);}event(room,'round-started',{round:round.id,mapId:round.mapId});continue;}
