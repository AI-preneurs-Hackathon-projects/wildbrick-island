...
export function createMotionView(){
 ...
 function update(next,dt,{correct=true}={}){if(!shown)accept(next);const length=Math.hypot(offset.x,offset.y,offset.z),limit=Math.max(2,next.speed*.55)*dt,decay=Math.min(1-Math.exp(-dt*12),length>0?limit/length:1),f=correct?1-decay:1;for(const key of ['x','y','z'])offset[key]*=f;shown={...next,x:next.x+offset.x,y:Math.max(0,next.y+offset.y),z:next.z+offset.z,yaw:next.yaw};return shown;}
