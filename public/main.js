...
async function bootGame(){
 ...
 const world=createWorld(scene),pilot=character();const actor=new THREE.Group();actor.add(pilot.group);scene.add(actor);
 ...
 const particleGeometry=new THREE.BoxGeometry(.20,.14,.23),pulseGeometry=new THREE.SphereGeometry(.23,10,8),particles=[],projectiles=[];let arena,arenaUI,arenaView,arenaMode=false;let assembly=null,sim,ui,input,voice,creations,activeGenerated=null,pendingGenerated=null,readyModel=null;const placedModels=[];let deviceStorage;try{deviceStorage=window.localStorage;}catch{}const adventure=createAdventure(deviceStorage),recentCreations=adventure.creations;let saveWarning=false;
 ...
 function updateMode(mode){
  ...
  if(mode==='bow'){equipment.bow.group.position.set(-.85,1.26,.64);equipment.bow.group.rotation.set(0,.05,0);}
 ...
 function startBuild(mode,custom){
  ...
  if(mode==='bow')g.position.set(-.85,1.26,.64);if(mode==='sword')g.position.set(.81,1.06,.37);
 ...
 const actions={state:()=>sim.state,openArena(){return arenaUI.play();},leaveArena(){return leaveArena();},start(){sound.enable();sim.state.started=true;saveAdventure();},build(type){sound.enable();if(arena?.active){creations.cancel();return arena.command(type==='foot'?'exit':'build',type);}if(!sim.state.building){creations.cancel();readyModel?.dispose();readyModel=null;}return sim.build(type);},action(){sound.enable();if(arena?.active)arena.command('fire');else sim.action();},jump(){sound.enable();if(arena?.active)arena.command('jump');else sim.jump();},voice(){if(!sim.state.started||sim.state.paused)return;sound.enable();voice.start();},describe(prompt){sound.enable();if(!sim.state.started){ui.toast('Start the adventure first.');return false;}return creations.generate(prompt);},cancelDesign(){creations.cancel();if(readyModel){readyModel.dispose();readyModel=null;}},recent:()=>recentCreations,saved:()=>adventure.available,rebuild(index){const b=recentCreations[index];if(!b)return;if(sim.state.building){ui.toast('Let these bricks finish assembling, then choose your saved creation.');return;}creations.cancel();try{queueBlueprint(b);}catch(e){ui.designError(e.message);}},sound:()=>sound.toggle(),pause(paused){sim.state.paused=paused;input?.clear();if(paused)voice?.stop();},respawn(){if(arena?.active){ui.toast('Arena respawns happen automatically after 12 seconds.');return;}sim.respawn();},};
 ui=createUI(actions);voice=createVoice({onCommand:actions.describe,onState:ui.voiceState,onNotice:ui.toast,onFallback:ui.voiceFallback});
 creations=createCreationService({onState:ui.generationState,onReady(blueprint){queueBlueprint(blueprint);adventure.remember(blueprint);saveAdventure();ui.connectionVerified();},onError:message=>ui.designError(message)});
 creations.status().then(ui.connectionState);
 arenaView=createArenaView(scene,camera);
 function returnToExplore(){if(!arenaMode)return;arenaMode=false;arenaView.clear();sim.state.arena=false;sim.state.paused=false;sim.state.autoRun=false;sim.state.building=null;sim.state.custom=null;sim.returnToFoot();updateMode('foot');actor.visible=true;placedModels.forEach(p=>p.model.group.visible=true);world.setDestroyed(Object.keys(sim.state.destroyed||{}));input?.clear();}
 async function leaveArena(){creations.cancel();const leaving=arena.leave();returnToExplore();sim.state.started=false;ui.showEntry();await leaving;}
 arena=createArenaClient({onStatus:value=>arenaUI?.setStatus(value),onSnapshot:s=>world.setDestroyed(Object.keys(s.destroyed)),onError:(message,status)=>arenaUI?.error(message,status),onEvent(e,self){const p=arena.snapshot?.players.find(p=>p.id===e.player);arenaView.effect({...e,x:e.x??p?.x,y:e.y??(p?.y||0)+1,z:e.z??p?.z});if(e.type==='attack-preview'||(!e.predicted&&['shot','swing'].includes(e.type)))sound.play(['punch','blade','knife','hammer'].includes(e.weapon)?'swing':e.weapon==='flame'?'flame':'shoot');if(e.type==='hit'){sound.play('hit');if(e.by===self)ui.toast('Hit · '+e.amount,700);}if(e.type==='blocked')sound.play('block');if(e.type==='respawn')sound.play('respawn');if(['break','crash','ko'].includes(e.type))sound.play(e.type==='ko'?'defeat':'break');if(e.type==='built'){sound.play('mode');if(e.player===self)ui.toast(e.name+' ready. '+(e.placed?'Placed nearby.':'→ attacks, ↑ jumps or rises, ↓ lowers, and ← starts speech.'),5000);}if(e.type==='notice'&&e.player===self)ui.toast(e.text,5500);if(e.type==='ko')ui.toast(e.killer+' knocked out '+e.name+'.',3500);if(e.type==='crash'&&e.player===self)ui.toast('Your creation broke apart! You’re back on foot.',4500);if(e.type==='pickup'&&e.player===self){sound.play('target');ui.toast(e.pickup==='health'?'Health restored!':e.pickup==='defense'?'Defense aura — 10 seconds!':'Double speed — 10 seconds!',4000);}}});
 arenaUI=createArenaUI({toast:ui.toast,leave:leaveArena,async join(name,room){creations.cancel();voice.stop();input?.clear();const ok=await arena.join(name,room);if(!ok)return false;if(!arenaMode){arenaMode=true;}restoreAssembly();assembly=null;for(const m of [activeGenerated,pendingGenerated,readyModel])m?.dispose();activeGenerated=pendingGenerated=readyModel=null;sim.state.building=null;sim.state.custom=null;sim.state.arena=true;sim.state.paused=false;sim.state.autoRun=false;projectiles.forEach(p=>p.mesh.removeFromParent());projectiles.length=0;placedModels.forEach(p=>p.model.group.visible=false);actor.visible=false;ui.toast('Arena joined. Hold → to punch. Speak a creation and watch for supply drops.',6500);return true;}});

 input=createInput({onBuild:actions.build,onAction:actions.action,onJump:actions.jump,onPause:()=>ui.toggleMenu('pause'),onMic:actions.voice,onImagine:()=>ui.openMenu('imagine'),onHotkeys:()=>ui.toggleMenu('hotkeys'),onHome:actions.respawn,getState:()=>sim.state});
 ...
 const frame=(time)=>{
  ...
  if(s.mode==='foot'||s.mode==='bow'||s.mode==='sword'){
   ...
   pilot.arms[1].rotation.x=s.mode==='sword'?-.25:running?Math.sin(s.time*12)*.52:0;
  ...
  if(activeGenerated){activeGenerated.update(s.time,1,s.speed/10,s.actionTime);if(s.custom?.blueprint.movement==='carry'){activeGenerated.group.rotation.z=-Math.sin(s.actionTime/.4*Math.PI)*1.2;gripOffset.copy(activeGenerated.grip).applyQuaternion(activeGenerated.group.quaternion);activeGenerated.group.position.set(.76,1.1,.3).sub(gripOffset);}}
  ...
  if(!s.paused){for(let i=particles.length-1;i>=0;i--){const p=particles[i];p.life-=dt;p.mesh.scale.setScalar(Math.min(1,p.life*3));if(p.life<=0){scene.remove(p.mesh);particles.splice(i,1);}}
   for(let i=projectiles.length-1;i>=0;i--){const p=projectiles[i];p.life+=dt;const t=Math.min(1,p.life/p.duration);p.mesh.position.lerpVectors(p.from,p.to,t);p.mesh.lookAt(p.to);if(t===1){if(p.id!==null)sim.hitTarget(p.id);if(p.pulse)makeBurst(p.to,p.color,12);scene.remove(p.mesh);projectiles.splice(i,1);}}
  ...
  else{const mobilePortrait=innerWidth<650&&innerHeight>innerWidth;const ak=arenaMode?arena.self?.kit:null,model=ak?.stats.mounted?{size:{x:ak.stats.collision[0],y:ak.stats.collision[1],z:ak.stats.collision[2]},seat:{y:ak.stats.collision[1]},blueprint:{movement:'fly'}}:activeGenerated||pendingGenerated,mounted=model&&model.blueprint.movement!=='carry';const base=s.mode==='plane'?21:s.mode==='car'?15:mobilePortrait?15:12;const height=mounted?Math.max(model.size.y,model.seat.y+1.6):2.4;const narrowFov=2*Math.atan(Math.tan(camera.fov*Math.PI/360)*Math.min(1,camera.aspect));const fit=mounted?Math.max(model.size.x,height,model.size.z)*.7/Math.tan(narrowFov/2):0;const distance=Math.max(base,Math.min(38,fit)),focusHeight=Math.max(1.1,height*.48),elev=input.pitch;desiredCamera.set(s.x-Math.sin(input.yaw)*distance,s.y+focusHeight+1.2+distance*Math.sin(elev),s.z-Math.cos(input.yaw)*distance);focus.set(s.x,s.y+focusHeight,s.z);if(arenaMode){const aim=input.read().aimPitch;desiredCamera.y=s.y+focusHeight+3;focus.set(s.x+Math.sin(input.yaw)*12,desiredCamera.y+Math.tan(aim)*(distance+12),s.z+Math.cos(input.yaw)*12);}cameraTarget.copy(focus);}
 ...
 if(document.modelContext?.registerTool){const lifecycle=new AbortController();for(const tool of [
  {name:'read_game_state',title:'Read Brickwild state',description:'Read current creation and island challenge progress.',inputSchema:{type:'object',properties:{},additionalProperties:false},annotations:{readOnlyHint:true},execute(){const s=sim.state;return {started:s.started,paused:s.paused,mode:s.mode,creation:s.custom?.blueprint.name??BUILDS[s.mode].name,designing:creations.pending,building:s.building?.custom?.blueprint.name??s.building?.mode??null,bricks:s.bricks,progress:{gates:s.gates.length,rings:s.rings.length,targets:s.targets.length,crates:s.crates.length}};}},
