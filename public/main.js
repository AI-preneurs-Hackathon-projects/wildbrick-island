...
async function bootGame(){
 ...
 function queueBlueprint(blueprint){if(arena?.active){arena.blueprints.set('pending',blueprint);const queued=arena.command('build','generated',blueprint);if(queued)ui.toast(blueprint.name+' designed. Assembling in the arena…',4500);return queued;}const next=createGeneratedModel(blueprint);readyModel?.dispose();readyModel=next;ui.toast(`${blueprint.name} designed. Getting the bricks ready…`,4500);}
 ...
 input=createInput({onPickup:actions.pickup,onDrop:actions.drop,onAction:actions.action,onJump:actions.jump,onPause:()=>ui.toggleMenu('pause'),onMic:actions.voice,onHotkeys:()=>ui.toggleMenu('hotkeys'),onCollection:()=>ui.openMenu('collection'),onSlot:actions.rebuild,getState:controlsState});
