...
async function bootGame(){
 ...
 let poseKit,poseBlueprint,poseMode;
 ...
 const actions={state:()=>sim.state,setAvatarColor(color){pilot.setColor(color);avatarPreview?.setColor(color);},exitHome(){const leaving=leaveArena(true);saveAdventure();sim.state.started=false;sim.state.autoRun=false;ui.showEntry();return leaving;},openArena(){return arenaUI.play();},leaveArena(){return leaveArena();},start(){sound.enable();sim.state.started=true;saveAdventure();},build(type){sound.enable();if(arena?.active){creations.cancel();return arena.command(type==='foot'?'exit':'build',type);}if(!sim.state.building){creations.cancel();readyModel?.dispose();readyModel=null;}return sim.build(type);},action(){sound.enable();if(arena?.active)arena.command('fire');else sim.action(input?.read());},jump(){sound.enable();if(arena?.active)arena.command('jump');else sim.jump();},voice(){if(!sim.state.started||sim.state.paused||arena?.snapshot?.round?.status==='finished')return;sound.enable();voice.start();},describe(prompt,timing=null){if(arena?.active&&arena.snapshot?.round?.status==='finished'){ui.toast('Restart the round to build again.');return false;}speechTiming=timing;sound.enable();if(!sim.state.started){ui.toast('Start the adventure first.');return false;}return creations.generate(prompt);},cancelDesign(){buildTiming=null;speechTiming=null;creations.cancel();if(readyModel){readyModel.dispose();readyModel=null;}},recent:()=>recentCreations,saved:()=>adventure.available,rebuild(index){const b=recentCreations[index];if(!b)return;if(sim.state.building){ui.toast('Let these bricks finish assembling, then choose your saved creation.');return;}creations.cancel();buildTiming={startedAt:performance.now(),requestMs:0,saved:true,assembled:false};speechTiming=null;try{queueBlueprint(b);}catch(e){ui.designError(e.message);}},sound:()=>sound.toggle(),pause(paused){sim.state.paused=paused;input?.clear();if(paused)voice?.stop();},respawn(){if(arena?.active){ui.toast('Arena respawns happen automatically after 12 seconds.');return;}sim.respawn();},};
 ...
 async function leaveArena(home=false){creations.cancel();readyModel?.dispose();readyModel=null;voice.stop();buildTiming=null;speechTiming=null;const leaving=arena.leave();returnToExplore();input?.clear();arenaUI.reset();ui.resetPlayUI();ui.update(sim.state);if(!home){document.querySelector('#open-arena').focus();ui.toast('Back in Explore. Your island progress is saved on this device.');}await leaving;}
