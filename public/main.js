...
async function bootGame(){
 ...
 const particleGeometry=new THREE.BoxGeometry(.20,.14,.23),pulseGeometry=new THREE.SphereGeometry(.23,10,8),particles=[],projectiles=[];let arena,arenaUI,arenaView,arenaMode=false;let assembly=null,sim,ui,input,voice,creations,activeGenerated=null,pendingGenerated=null,readyModel=null;const placedModels=[];let buildTiming=null,speechTiming=null;let deviceStorage;try{deviceStorage=window.localStorage;}catch{}const adventure=createAdventure(deviceStorage),recentCreations=adventure.creations;let saveWarning=false;
 ...
 async function leaveArena(){creations.cancel();voice.stop();buildTiming=null;speechTiming=null;const leaving=arena.leave();returnToExplore();input?.clear();arenaUI.reset();ui.resetPlayUI();ui.update(sim.state);document.querySelector('#open-arena').focus();ui.toast('Back in Explore. Your island progress is saved on this device.');await leaving;}
 arena=createArenaClient({onStatus:value=>arenaUI?.setStatus(value),onSnapshot:s=>world.setDestroyed(Object.keys(s.destroyed)),onError:(message,status)=>arenaUI?.error(message,status),onEvent(e,self){const p=arena.snapshot?.players.find(p=>p.id===e.player);const local=arena.self,near=e.player===self||e.by===self||!local||Math.hypot((e.x??p?.x??0)-local.x,(e.y??p?.y??0)-local.y,(e.z??p?.z??0)-local.z)<35;const play=type=>{if(near)sound.play(type);};arenaView.effect({...e,x:e.x??p?.x,y:e.y??(p?.y||0)+1,z:e.z??p?.z});if(e.type==='attack-preview'||(!e.predicted&&['shot','swing'].includes(e.type)))play(['punch','blade','knife','hammer'].includes(e.weapon)?'swing':e.weapon==='flame'?'flame':'shoot');if(e.type==='hit'){play('hit');if(e.by===self)ui.toast('Hit · '+e.amount,700);}if(e.type==='blocked')play('block');if(e.type==='respawn')play('respawn');if(['break','crash','ko'].includes(e.type))play(e.type==='ko'?'defeat':'break');if(e.type==='built'){if(e.player===self&&buildTiming){buildTiming.usableMs=performance.now()-buildTiming.startedAt;buildTiming.assembled=true;}play('mode');if(e.player===self)ui.toast(e.name+' ready. '+(e.placed?'Placed nearby.':'→ attacks, ↑ jumps or rises, ↓ lowers, and ← starts speech.'),5000);}if(e.type==='airdrop')ui.toast(e.name+' incoming — look for a violet parachute.',5000);if(e.type==='notice'&&e.player===self)ui.toast(e.text,5500);if(e.type==='ko')ui.toast(e.killer+' knocked out '+e.name+'.',3500);if(e.type==='crash'&&e.player===self)ui.toast('Your creation broke apart! You’re back on foot.',4500);if(e.type==='pickup'&&e.player===self){play('target');ui.toast(e.pickup==='equipment'?e.name+' equipped! Hold → to attack; Backspace returns to foot.':e.pickup==='health'?'Health restored!':e.pickup==='defense'?'Defense aura — 10 seconds!':'Double speed — 10 seconds!',4000);}}});
 arenaUI=createArenaUI({getPlayerName:ui.playerName,leave:leaveArena,toast:ui.toast,async join(name,room){creations.cancel();voice.stop();input?.clear();const ok=await arena.join(name,room);if(!ok)return false;if(!arenaMode){arenaMode=true;}restoreAssembly();assembly=null;for(const m of [activeGenerated,pendingGenerated,readyModel])m?.dispose();activeGenerated=pendingGenerated=readyModel=null;sim.state.building=null;sim.state.custom=null;sim.state.arena=true;sim.state.paused=false;sim.state.autoRun=false;projectiles.forEach(p=>p.mesh.removeFromParent());projectiles.length=0;placedModels.forEach(p=>p.model.group.visible=false);actor.visible=false;ui.toast('Arena joined. Hold → to punch. Speak a creation and watch for supply drops.',6500);return true;}});

 input=createInput({onBuild:actions.build,onAction:actions.action,onJump:actions.jump,onPause:()=>ui.toggleMenu('pause'),onMic:actions.voice,onHotkeys:()=>ui.toggleMenu('hotkeys'),onHome:actions.respawn,getState:()=>sim.state});
