...
async function bootGame(){
 ...
 const particleGeometry=new THREE.BoxGeometry(.20,.14,.23),pulseGeometry=new THREE.SphereGeometry(.23,10,8),particles=[],projectiles=[];let arena,arenaUI,arenaView,arenaMode=false;let assembly=null,sim,ui,input,voice,creations,activeGenerated=null,pendingGenerated=null,readyModel=null;const placedModels=[];let buildTiming=null;let deviceStorage;try{deviceStorage=window.localStorage;}catch{}const adventure=createAdventure(deviceStorage),recentCreations=adventure.creations;let saveWarning=false;
 ...
 creations=createCreationService({onState:ui.generationState,onReady(blueprint,prompt,timing){buildTiming={...timing,assembled:false};queueBlueprint(blueprint);adventure.remember(blueprint);saveAdventure();ui.connectionVerified();},onError:message=>ui.designError(message)});
