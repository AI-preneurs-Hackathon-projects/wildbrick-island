...
import {nearbyItem} from './arena-core.js?v=37';
import {meleeRotation} from './combat-pose.js';
import {gameCamera} from './game-camera.js';
import {createAvatarPreview} from './avatar-preview.js?v=37';
...
import {createArenaClient} from './arena-client.js?v=40';
import {createArenaView} from './arena-view.js';
import {createArenaUI} from './arena-ui.js?v=40';
...
async function bootGame(){
 ...
 function onEvent(event){
  ...
  if(event.type==='mode'){practiceShot=null;practiceAimGuide.clear();ui.buildComplete();if(activeGenerated){activeGenerated.dispose();activeGenerated=null;}if(event.custom){activeGenerated=pendingGenerated;pendingGenerated=null;}updateMode(event.mode);}
  ...
  if(event.type==='smash'){sim.removeEntity('crate:'+event.id);const g=world.crates[event.id];g.visible=false;makeBurst(new THREE.Vector3(CRATES[event.id].x,1.1,CRATES[event.id].z),C.purple,30);ui.toast(`Smashing! ${sim.state.crates.length}/3 crates`);}
  if(event.type==='target'){const g=world.targets[event.id];g.scale.setScalar(.82);g.rotation.z=.2;makeBurst(new THREE.Vector3(TARGETS[event.id].x,1.9,TARGETS[event.id].z),C.teal,22);ui.toast(`Bullseye! ${sim.state.targets.length}/3 targets`);}
  if(event.type==='gate'){const g=world.gates[event.id];g.traverse(m=>{if(m.isMesh&&m.material.color.getHex()===C.teal)m.material=material(C.yellow);});makeBurst(new THREE.Vector3(GATES[event.id].x,2,GATES[event.id].z),C.teal,25);ui.toast(`Nice driving! ${sim.state.gates.length}/4 gates`);}
  if(event.type==='ring'){sim.removeEntity('ring:'+event.id);world.rings[event.id].visible=false;makeBurst(new THREE.Vector3(RINGS[event.id].x,RINGS[event.id].y,RINGS[event.id].z),C.yellow,35);ui.toast(`Sky high! ${sim.state.rings.length}/5 rings`);}
  if(event.type==='win')ui.toast('Every challenge complete! Reset the Practice route whenever you want to play it again.',5500);
 }
 ...
 const controlsState=()=>({...sim.state,...(arena?.active?{health:arena.self?.health,building:arena.self?.building,roundFinished:arena.snapshot?.round?.status==='finished',carrying:arena.self?.kit?.id!=='foot'}:{carrying:sim.state.mode!=='foot'||!!sim.state.custom})});
 const itemActionAllowed=()=>!gameplayBlocked(controlsState())&&!controlsState().building&&!document.querySelector('dialog[open]');
 let snapshotBusy=false;
 const pngBlob=canvas=>new Promise((resolve,reject)=>canvas.toBlob(blob=>blob?resolve(blob):reject(Error('The snapshot could not be created.')),'image/png'));
 async function saveSnapshot(){
  if(snapshotBusy)return false;snapshotBusy=true;const button=document.querySelector('#snapshot');if(button)button.disabled=true;
  try{const blob=await pngBlob(composeGameSnapshot(renderer,scene,camera));
   const url=URL.createObjectURL(blob),link=document.createElement('a');link.href=url;link.download='brickwild-snapshot-'+new Date().toISOString().slice(0,19).replaceAll(':','-')+'.png';document.body.appendChild(link);link.click();link.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);ui.toast('Snapshot saved.',2500);return true;
  }catch{ui.toast('The snapshot could not be saved. Try again.',4000);return false;}finally{if(button)button.disabled=false;snapshotBusy=false;}
 }
 const actions={state:controlsState,snapshot:saveSnapshot,cancelBuild(){const canceled=arena?.active?arena.command('cancel-build'):sim.cancelBuild();if(!canceled)return false;restoreAssembly();assembly=null;if(pendingGenerated){pendingGenerated.dispose();pendingGenerated=null;}updateMode(sim.state.mode);ui.update(controlsState());return true;},drop(){if(!itemActionAllowed())return false;if(arena?.active)return arena.command('exit');return actions.build('foot');},pickup(id){if(!itemActionAllowed())return false;if(!arena?.active){ui.toast('Recoverable items are available in Arena. Rebuild saved creations from Your creations.');return false;}if(arena.self.kit.id!=='foot'){ui.toast('Press '+bindingLabel('drop')+' to drop your current item before picking up another.');return false;}const target=nearbyItem(arena.snapshot,arena.self);if(!target){ui.toast('No recoverable item nearby. Move closer to an item.');return false;}return arena.command('pickup',id||target.id);},setAvatarColor(color){pilot.setColor(color);avatarPreview?.setColor(color);},resetPractice(){if(arena?.active)return;sim.resetChallenges();world.rings.forEach(g=>g.visible=true);world.crates.forEach(g=>g.visible=true);world.targets.forEach(g=>{g.scale.setScalar(1);g.rotation.z=0;});world.gates.forEach(g=>g.traverse(m=>{if(m.isMesh&&m.material.color.getHex()===C.yellow)m.material.color.setHex(C.teal);}));saveAdventure();ui.update(sim.state);ui.toast('Practice route reset. All four challenges are ready again.',4500);},exitHome(){const leaving=leaveArena(true);saveAdventure();sim.state.started=false;sim.state.autoRun=false;ui.showEntry();return leaving;},openArena(){return arenaUI.play();},leaveArena(){return leaveArena();},start(){sound.enable();sim.state.started=true;saveAdventure();},build(type){sound.enable();if(arena?.active){creations.cancel();return arena.command(type==='foot'?'exit':'build',type);}if(!sim.state.building){creations.cancel();readyModel?.dispose();readyModel=null;}return sim.build(type);},action(){sound.enable();if(arena?.active)arena.command('fire');else sim.action(input?.read());},jump(){sound.enable();if(arena?.active)arena.command('jump');else sim.jump();},recordVoice(){if(!sim.state.started||sim.state.paused||controlsState().health<=0||arena?.snapshot?.round?.status==='finished'||document.querySelector('dialog[open]'))return;voice.record();},cancelVoice(){voice.stop();},voice(){if(!sim.state.started||sim.state.paused||controlsState().health<=0||arena?.snapshot?.round?.status==='finished'||document.querySelector('dialog[open]'))return;sound.enable();voice.start();},describe(prompt,timing=null){if(arena?.active&&arena.snapshot?.round?.status==='finished'){ui.toast('The next round starts after the scoreboard.');return false;}speechTiming=timing;sound.enable();if(!sim.state.started){ui.toast('Start the adventure first.');return false;}return creations.generate(prompt);},cancelDesign(){buildTiming=null;speechTiming=null;creations.cancel();if(readyModel){readyModel.dispose();readyModel=null;}},recent:()=>creationSlots(recentCreations),saved:()=>adventure.available,rebuild(index){const b=creationSlots(recentCreations)[index];if(!b)return;if(sim.state.building){ui.toast('Let these bricks finish assembling, then choose your saved creation.');return;}creations.cancel();buildTiming={startedAt:performance.now(),requestMs:0,saved:true,assembled:false};speechTiming=null;try{queueBlueprint(b);}catch(e){ui.designError(e.message);}},sound:()=>sound.toggle(),pause(paused){sim.state.paused=paused;input?.clear();if(paused)voice?.stop();},respawn(){if(arena?.active){ui.toast('Arena respawns happen automatically after 12 seconds.');return;}sim.respawn();},};
