...
function createIslandWorld(scene){
 ...
 const ocean=new THREE.Mesh(new THREE.PlaneGeometry(1600,1600),new THREE.MeshStandardMaterial({color:0xa4d8e3,roughness:.92,metalness:0}));ocean.material.userData.worldOwned=true;ocean.rotation.x=-Math.PI/2;ocean.position.y=-2.25;ocean.receiveShadow=true;scene.add(ocean);
 const waves=new THREE.Group();for(let i=0;i<100;i++){const x=Math.sin(i*2.13)*(78+i%7*18),z=Math.cos(i*3.79)*(78+i%9*14);if(Math.abs(x)<62&&Math.abs(z)<62)continue;box(waves,x,-2.20,z,1.8+i%5,.025,.14,0xd9f1f0);}scene.add(batchStatic(waves));
 const clouds=new THREE.Group();for(let i=0;i<12;i++){const angle=i*2.4,radius=88+(i%4)*11,g=new THREE.Group();g.position.set(Math.sin(angle)*radius,14+(i%5)*3.5,Math.cos(angle)*radius);g.rotation.y=angle;brick(g,-3.6,0,0,7.2,2.4,4.8,C.white,false);brick(g,1.4,.9,(i%2?-.3:.3),8,3.5,5.4,C.white,false);brick(g,5.6,.15,(i%2?.2:-.2),5.6,2.1,4.4,C.white,false);clouds.add(g);}scene.add(batchStatic(clouds));
