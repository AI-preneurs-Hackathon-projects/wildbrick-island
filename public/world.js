...
function createIslandWorld(scene){
 ...
 const ocean=new THREE.Mesh(new THREE.PlaneGeometry(1600,1600),new THREE.MeshStandardMaterial({color:0xb4dedd,roughness:.27,metalness:.12}));ocean.material.userData.worldOwned=true;ocean.rotation.x=-Math.PI/2;ocean.position.y=-2.25;ocean.receiveShadow=true;scene.add(ocean);
 const waves=new THREE.Group();for(let i=0;i<100;i++){const x=Math.sin(i*2.13)*(78+i%7*18),z=Math.cos(i*3.79)*(78+i%9*14);if(Math.abs(x)<62&&Math.abs(z)<62)continue;box(waves,x,-2.20,z,1.8+i%5,.025,.14,0xcde9e7);}scene.add(batchStatic(waves));
