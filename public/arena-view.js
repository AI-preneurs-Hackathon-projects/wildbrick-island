...
import {weaponGrip,aimDirection} from './weapon-aim.js';
...
export function createArenaView(scene,camera){
 ...
 g.name='arena-player:'+p.id;g.add(pilot.group,aura,label);root.add(g);const a={g,pilot,label,canvas,texture,aura,arc,flash,model:null,modelId:null,pending:null,pendingId:null,lastLabel:'',position:new THREE.Vector3(p.x,p.y,p.z)};g.add(arc,flash);actors.set(p.id,a);return a;}
...
if(a.modelId!==p.kit.id||(!a.model&&p.kit.blueprintId&&cache.has(p.kit.blueprintId))){a.model?.dispose();a.model=kitModel(p.kit,cache);a.modelId=p.kit.id;if(a.model){a.model.group.name='creation';a.g.add(a.model.group);}}
  ...
  if(model&&p.kit.stats.projectileSpeed>0&&!p.kit.stats.mounted){const confirmed=action&&time<action.poseUntil&&action.muzzle&&action.kitId===p.kit.id;poseRangedModel(model,p,confirmed?{yaw:action.aimYaw,muzzle:action.muzzle}:solution);}
  ...
  if(a.flash.visible){const from=action?.origin;if(from){a.g.updateWorldMatrix(true,false);a.flash.position.copy(a.g.worldToLocal(new THREE.Vector3(from.x,from.y,from.z)));}else{a.flash.visible=false;}const flame=p.kit.stats.weapon==='flame';a.flash.scale.set(flame?2:1,flame?2:1,flame?5:1.8);a.flash.rotation.set(-(action?.pitch||0),(action?.yaw??aimYaw)-a.g.rotation.y,0,'YXZ');a.flash.material.color.set(flame?0xff7836:0xffed9a);}
 ...
 function update(snapshot,local,cache,dt,time,options={}){viewTime=time;root.visible=!!snapshot;guide.clear();if(!snapshot)return;const boxes=snapshot===worldSnapshot?worldBoxes:(worldSnapshot=snapshot,worldBoxes=solidBoxes(snapshot)),live=new Set();snapshot.players.forEach((remote,i)=>{const p=remote.id===snapshot.self&&local?local:remote;live.add(p.id);const a=actors.get(p.id)||createActor(p,i);if(p.health>0&&a.dead)collapse.fade(p.id);const dying=p.health<=0&&!a.dead;a.g.visible=true;a.position.lerp(new THREE.Vector3(p.x,p.y,p.z),p.id===snapshot.self?1:1-Math.exp(-dt*12));a.g.position.copy(a.position);a.g.rotation.y=p.id===snapshot.self?p.yaw:a.g.rotation.y+Math.atan2(Math.sin(p.yaw-a.g.rotation.y),Math.cos(p.yaw-a.g.rotation.y))*(1-Math.exp(-dt*14));const own=p.id===snapshot.self;if(own||a.aimSnapshot!==snapshot||a.aimFixed!==options.fixedAim){a.aim=solveWeaponAim(snapshot,p,{boxes,fixed:options.fixedAim});a.aimSnapshot=snapshot;a.aimFixed=options.fixedAim;}renderKit(a,p,cache,time,a.aim);if(own&&options.showGuide!==false&&p.health>0&&!p.building&&snapshot.round?.status!=='finished'&&a.aim.active){
    const action=attacks.get(p.id),held=action&&time<action.poseUntil&&action.kitId===p.kit.id;let path;
    if(held){const direction=aimDirection(action.aimYaw),from=action.origin,muzzle=action.muzzle,launch=Math.hypot(from.x-muzzle.x,from.y-muzzle.y,from.z-muzzle.z)>1e-7;
     if(launch){const stop=playback.shots.get(action.attack)?.stop,contact=projectileContact(snapshot,{owner:p.id,weapon:p.kit.stats.weapon},from,muzzle,boxes),t=contact?.t??1;path={from,to:stop||{x:from.x+(muzzle.x-from.x)*t,y:from.y+(muzzle.y-from.y)*t,z:from.z+(muzzle.z-from.z)*t},direction,spread:0,launch:true};}
     else{path=weaponPath(snapshot,p,{...a.aim,muzzle,direction,launchContact:null},boxes);const stop=playback.shots.get(action.attack)?.stop;if(stop)path={...path,to:stop};}
    }else path=weaponPath(snapshot,p,a.aim,boxes);
    guide.update(path,camera);
 ...
 function effect(e){
  ...
  if(['attack-preview','shot','swing'].includes(e.type)){const old=attacks.get(e.player);if(e.predicted&&e.type!=='shot')return;attacks.set(e.player,{time:e.predicted&&old?old.time:viewTime||e.time,weapon:e.weapon,yaw:e.yaw??0,pitch:e.pitch||0,origin:e.type==='shot'?{x:e.x,y:e.y,z:e.z}:e.origin,aimYaw:e.aimYaw??e.yaw??0,kitId:e.kitId,attack:e.projectile?.id,muzzle:e.muzzle||null,poseUntil:e.muzzle?(e.predicted&&old?old.time:viewTime||e.time)+100:0});return;}
  if(['hit','blocked'].includes(e.type))reactions.set(e.player,{time:viewTime||e.time,type:e.type});
  if(['round-ended','round-started','ko','leave'].includes(e.type)){guide.clear();attacks.delete(e.player);}
