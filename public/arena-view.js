...
export function createArenaView(scene,camera){
 const playback=createShotPlayback();const root=new THREE.Group();root.visible=false;scene.add(root);const collapse=createBrickCollapse(root);const actors=new Map(),shots=new Map(),drops=new Map(),placed=new Map(),items=new Map(),effects=[];let viewTime=0;const attacks=new Map(),reactions=new Map();
...
if(a.modelId!==p.kit.id||(!a.model&&p.kit.blueprintId&&cache.has(p.kit.blueprintId))){a.model?.dispose();a.model=kitModel(p.kit,cache);a.modelId=p.kit.id;if(a.model){a.model.group.name='creation';a.g.add(a.model.group);}}
 ...
 function update(snapshot,local,cache,dt,time){viewTime=time;root.visible=!!snapshot;if(!snapshot)return;const live=new Set();snapshot.players.forEach((remote,i)=>{const p=remote.id===snapshot.self&&local?local:remote;live.add(p.id);const a=actors.get(p.id)||createActor(p,i);if(p.health>0&&a.dead)collapse.fade(p.id);const dying=p.health<=0&&!a.dead;a.g.visible=true;a.position.lerp(new THREE.Vector3(p.x,p.y,p.z),p.id===snapshot.self?1:1-Math.exp(-dt*12));a.g.position.copy(a.position);a.g.rotation.y=p.id===snapshot.self?p.yaw:a.g.rotation.y+Math.atan2(Math.sin(p.yaw-a.g.rotation.y),Math.cos(p.yaw-a.g.rotation.y))*(1-Math.exp(-dt*14));renderKit(a,p,cache,time);if(dying)collapse.scatter(p.id,a.pilot.group);a.dead=p.health<=0;a.g.visible=!a.dead;
  ...
  collapse.update(dt,solidBoxes(snapshot).map(item=>item.box));
