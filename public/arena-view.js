...
export function createArenaView(scene,camera){
 const playback=createShotPlayback();const root=new THREE.Group();root.visible=false;scene.add(root);const actors=new Map(),shots=new Map(),drops=new Map(),placed=new Map(),effects=[];let viewTime=0;const attacks=new Map(),reactions=new Map();
 ...
 function renderKit(a,p,cache,time){
  const aimYaw=p.aimYaw??p.yaw,aimPitch=p.aimPitch||0;const action=attacks.get(p.id),elapsed=action?(time-action.time)/1000:100,pose=combatPose(p.kit.stats.weapon,elapsed),strike=pose.strike;
if(a.modelId!==p.kit.id||(!a.model&&p.kit.blueprintId&&cache.has(p.kit.blueprintId))){a.model?.dispose();a.model=kitModel(p.kit,cache);a.modelId=p.kit.id;if(a.model){a.model.group.name='creation';a.g.add(a.model.group);}}
 ...
 function update(snapshot,local,cache,dt,time){viewTime=time;root.visible=!!snapshot;if(!snapshot)return;const live=new Set();snapshot.players.forEach((remote,i)=>{const p=remote.id===snapshot.self&&local?local:remote;live.add(p.id);const a=actors.get(p.id)||createActor(p,i);a.g.visible=p.health>0;if(!a.g.visible)return;a.position.lerp(new THREE.Vector3(p.x,p.y,p.z),p.id===snapshot.self?1:1-Math.exp(-dt*12));a.g.position.copy(a.position);a.g.rotation.y=p.id===snapshot.self?p.yaw:a.g.rotation.y+Math.atan2(Math.sin(p.yaw-a.g.rotation.y),Math.cos(p.yaw-a.g.rotation.y))*(1-Math.exp(-dt*14));renderKit(a,p,cache,time);
  ...
  for(const impact of playback.update(dt,solidBoxes(snapshot).map(item=>item.box)))effect({...impact,played:true});
  for(const [id,b] of playback.shots){let m=shots.get(id);if(!m){m=new THREE.Mesh(b.weapon==='flame'?fireGeo:shotGeo,new THREE.MeshBasicMaterial({color:['automatic','bow'].includes(b.weapon)?'#ffed9a':b.color||'#ffcf55',toneMapped:false}));m.name='projectile:'+id;shots.set(id,m);root.add(m);}m.position.set(b.position.x,b.position.y,b.position.z);m.scale.set(b.weapon==='flame'?2:1.2,b.weapon==='flame'?2:1.2,b.weapon==='flame'?3:7);m.lookAt(m.position.x+b.vx,m.position.y+b.vy,m.position.z+b.vz);}
 ...
 function effect(e){
  ...
  if(['attack-preview','shot','swing'].includes(e.type)){attacks.set(e.player,{time:viewTime||e.time,weapon:e.weapon,yaw:e.yaw??0,pitch:e.pitch||0,origin:e.type==='shot'?{x:e.x,y:e.y,z:e.z}:e.origin});return;}
 ...
 function clear(){playback.clear();update({players:[],projectiles:[],drops:[],placed:[],destroyed:{}},null,new Map(),0,0);root.visible=false;for(const e of effects){e.mesh.geometry.dispose();if(e.ownedMaterial)e.mesh.material.dispose();e.mesh.removeFromParent();}effects.length=0;attacks.clear();reactions.clear();}
