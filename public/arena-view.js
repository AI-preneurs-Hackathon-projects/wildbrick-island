...
if(a.modelId!==p.kit.id||(!a.model&&p.kit.blueprintId&&cache.has(p.kit.blueprintId))){a.model?.dispose();a.model=kitModel(p.kit,cache);a.modelId=p.kit.id;if(a.model){a.model.group.name='creation';a.g.add(a.model.group);}}
 ...
 function effect(e){
  ...
  if(['attack-preview','shot','swing'].includes(e.type)){if(e.predicted)return;attacks.set(e.player,{time:viewTime||e.time,weapon:e.weapon,yaw:e.yaw??0,pitch:e.pitch||0,origin:e.type==='shot'?{x:e.x,y:e.y,z:e.z}:e.origin});return;}
