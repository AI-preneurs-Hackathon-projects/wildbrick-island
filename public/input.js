...
export function createInput({onBuild,onAction,onJump,onPause,onMic,onHotkeys=()=>{},onHome,getState}){
 ...
 function clear(){keys.clear();stick.x=stick.z=0;up=down=fire=false;drag=null;joyPointer=null;knob.style.transform='translate(0,0)';}
 ...
 function update(dt){if(blocked())return;const step=Math.min(.1,dt);yaw+=((keys.has('KeyQ')?1:0)-(keys.has('KeyE')?1:0))*1.8*step;}
 return {clear,update,get yaw(){return yaw;},get pitch(){return pitch;},read(){return {x:stick.x+(keys.has('KeyD')?1:0)-(keys.has('KeyA')?1:0),z:stick.z+(keys.has('KeyW')?1:0)-(keys.has('KeyS')?1:0),cameraYaw:yaw,weaponPitch:0,fire:fire||keys.has('ArrowRight'),up:up||keys.has('ArrowUp')||keys.has('Space'),down:down||keys.has('ArrowDown'),sprint:keys.has('ShiftLeft')||keys.has('ShiftRight')};}};
}
