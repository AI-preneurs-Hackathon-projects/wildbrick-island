...
export function createInput({onBuild,onDrop=()=>onBuild?.('foot'),onAction=()=>{},onJump=()=>{},onPause=()=>{},onMic=()=>{},onHotkeys=()=>{},onCollection=()=>{},onSlot=()=>{},onPickup=()=>false,onHome,getState}){
 ...
 window.addEventListener('keydown',e=>{
  ...
  if(!e.repeat&&(!menu||menu.id==='menu')&&['help','menu'].includes(action)){
   e.preventDefault();clear();if(action==='help')onHotkeys();else onPause();return;
  }
  ...
  if(ACTION_BINDINGS[action]?.slot!==undefined){if(!getState().building)onSlot(ACTION_BINDINGS[action].slot);return;}
 ...
 function update(dt){fireFrame=fireTap;fireTap=false;if(blocked()){clear();return;}const step=Math.min(.1,dt);yaw+=((held('lookLeft')?1:0)-(held('lookRight')?1:0))*1.8*step;pitch=Math.max(-.75,Math.min(.92,pitch+((held('pitchDown')?1:0)-(held('pitchUp')?1:0))*1.35*step));}
