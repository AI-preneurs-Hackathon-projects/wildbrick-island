...
export function createCreationService({onState,onReady,onError,timeoutMs=95000}){
 ...
 return {generate,cancel,get pending(){return !!current;},async status(){try{const response=await fetch('/api/generation-status');if(!response.ok)return false;const d=await response.json();return d.configured===true;}catch{return false;}}};
}
