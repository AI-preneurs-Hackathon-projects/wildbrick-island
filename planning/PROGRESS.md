...
- Part A committed as `4bb9f3c`. Part B completed: shared Down/G/T binding definitions, guarded independent keyboard/touch actions, recoverable-item preservation and matching HUD/help/aria hints. Six new control groups plus existing UI/control/round checks pass; harness adds descent/drop/fall/pickup cases. Part B committed separately before strength changes.
- Part B committed as `ccbec6d`. Part C completed: punch 27, sword 42, knife 20, hammer 60; unchanged cadence/reach and ranged values. Five focused balance groups and 108 controlled family/profile/rule-set measurements support the candidate; old persisted stats are preserved until ordinary reconstruction/reset paths. See `COMBAT_BALANCE_CANDIDATE.md`.
- Final combined full suite/build/package passed (63 assets, same three migrations, development fixtures excluded, Arena 401). All 17 aiming/44 contact/5 emitter groups, 9 delayed fixtures, new 6 attachment/6 controls/5 balance groups and 42 harness interactions pass. No new cloud pixels or real hosted pair acceptance. Combined portable patch targets d874772 and preserves both identities; local commits only. Exact controls, changed files and remaining review: `CRITICAL_GAMEPLAY_REVIEW.md`.
