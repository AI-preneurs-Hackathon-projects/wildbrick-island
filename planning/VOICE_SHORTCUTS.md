...
## Authorized release and acceptance follow-up — 2026-09-14

Yerzhan explicitly authorized deployment and the previously proposed $1 test budget. GitHub main refreshed unchanged at `e9542efcacb20780e12683350c56c0204668c394`. Creator source `e235d3de0beb1832cbce4621c264e271f76aac89` was pushed and deployed as **version20**, deployment `appgdep_6aa7b8f854d081919e3fb9e75a2d2b73`, terminal **succeeded**, using environment revision **7**. Audience remains custom revision3; geometry model/settings and databases preserved. This supersedes the earlier pending-publication checkpoint.

The actual `/__layouts` browser review exposed overlap between a long transcript and saved slots at 768×1024. The bounded correction raises tablet voice status/recording-offer panels above the slot area. The development layout harness now measures actual rendered box intersection at 1024, 768 and 390px; screenshots and before/after measurements accompany the release receipt. No aim, movement, damage or networking changes.

Live paid-test preparation used existing local libflite to synthesize a benign 2.303-second, 10,139-byte WebM clip. The live `/api/transcription-status` request using the existing owner preview credential returned **401**: that credential did not establish the route's required trusted signed-in principal. No audio upload, transcription submission, generation submission or paid retry followed. The synthetic clip was deleted. **Budget approved; paid usage from this test: $0. Configuration is deployed, but authenticated transcription success is still unverified.** No identity header was fabricated and no authentication/access policy was weakened to make the test pass.

The cloud no-upload voice harness offered no usable real-microphone recorder (Record instead disabled, zero mock uploads and zero creation callbacks). No second authorized signed-in player or Brave runtime is available here. Human Chrome/Brave voice, real match transitions/two-player acceptance and WebGL gameplay remain blocked by environment/account availability, not spending approval. Use the existing local checklist with authorized signed-in players; no further budget approval is needed within the already approved six clips/two designs/$1 envelope. Refresh costs if that envelope changes.

Final correction evidence: tablet overlap was **2102.1875 px² before**, then **0 px²** after. The actual rendered harness reports zero slot/transcript overlap at 1024×768, 768×1024 and 390×844; the existing six speech states retain zero microphone-button shift at all three widths. `voice-shortcuts/tablet-voice-clearance.png` shows the corrected tablet panel clear of the slot strip. Full `npm run check` passed again after this CSS correction. Physical touch and 200% text zoom remain separate local checks.
