...
### Accepted production release — 2026-09-15

- Yerzhan and Hadrien manually tested the Render-backed preview and accepted it for production.
- Before merging, published backup release `backup/main-before-realtime-2026-09-15` at `dd5dab9`. Actual previous production is separately tagged `backup/production-before-realtime-2026-09-15` at `9860d1d`; its Vercel deployment is `dpl_AdqR7bC87RR8bqjBuVAbniGeugik`.
- Merge includes the complete accepted `5d9b2bd` feature branch. Production retains its existing Turso, session identity and AI settings. Added dedicated `wildbrick:production:arena` Redis namespace and a new production ticket secret; preview keys remain isolated.
- Only additive migrations are present; production build applies missing migrations transactionally. Never reset, drop, delete, or restart a database.
- Rollback: restore the previous production Vercel deployment above. Its deployment-time environment and HTTP transport are preserved. Do not deploy the old main tag as a substitute: old main predates the Vercel hosting integration. Retain additive schema changes and existing saved creations.
- Preview live admission/input acknowledgement/late join/refresh/exit-rejoin/cleanup passed after an initially stalled attempt. Production deployment and a short public smoke are the final release checks, not another broad playtest.
...
- Yerzhan explicitly approved external access on this new Redis only. Applied the public IP allow-list with mandatory password/TLS; authenticated TLS PING returned PONG. No existing database was restarted or changed.
- Merged Hadrien's `bc0c65d` feature-branch fixes without merging the historical performance branch. Focused realtime client and 24 Arena UI assertions plus the Vercel asset build pass.
- Preview deployment and short live gameplay connectivity verification follow. Human acceptance remains with Yerzhan and Hadrien; main and production remain unchanged.
