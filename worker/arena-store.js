...
// Serialize same-isolate writes; CAS remains the cross-isolate authority.
const queues=new WeakMap();
async function serialized(db,id,fn){
 let rooms=queues.get(db);if(!rooms){rooms=new Map();queues.set(db,rooms);}
 const prior=rooms.get(id)||Promise.resolve();
 const task=prior.catch(()=>{}).then(fn);rooms.set(id,task);
 try{return await task;}finally{if(rooms.get(id)===task)rooms.delete(id);}
}
export function arenaStore(db){if(!db)throw new ArenaError('The shared arena is unavailable. Please try Play again shortly.',503);
 return {
  async mutate(roomId,fn){return serialized(db,roomId,async()=>{const started=Date.now();for(let attempt=0;attempt<16&&Date.now()-started<1800;attempt++){
