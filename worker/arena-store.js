...
// Keep request I/O within its own Worker invocation. A canceled request must
...
export function arenaStore(db){if(!db)throw new ArenaError('The shared arena is unavailable. Please try Play again shortly.',503);
 return {
  ...
  async saveSession(s){const now=Date.now();await db.prepare('INSERT INTO arena_sessions (id, token_hash, principal, room_id, player_id, expires_at, rate_start, rate_count) SELECT ?, ?, ?, ?, ?, ?, ?, 0 WHERE (SELECT COUNT(*) FROM arena_sessions WHERE principal = ? AND expires_at > ?) < 12 ON CONFLICT(id) DO NOTHING').bind(s.id,s.tokenHash,s.principal,s.roomId,s.playerId,now+90000,now,s.principal,now).run();
   const row=await db.prepare('SELECT * FROM arena_sessions WHERE id = ?').bind(s.id).first();if(!row)throw new ArenaError('Close an older arena session before joining again.',429);
   if(row.principal!==s.principal||row.token_hash!==s.tokenHash||row.room_id!==s.roomId)throw new ArenaError('This join attempt is no longer valid. Exit to home and try again.',401);
   const allowed=await db.prepare('UPDATE arena_sessions SET rate_count = CASE WHEN rate_start < ? THEN 1 ELSE rate_count + 1 END, rate_start = CASE WHEN rate_start < ? THEN ? ELSE rate_start END, expires_at = ? WHERE id = ? AND (rate_start < ? OR rate_count < 100) AND (expires_at > ? OR (SELECT COUNT(*) FROM arena_sessions WHERE principal = ? AND expires_at > ?) < 12) RETURNING id').bind(now-10000,now-10000,now,now+90000,s.id,now-10000,now,s.principal,now).first();if(!allowed)throw new ArenaError('Close an older Arena session or wait a moment before joining again.',429);
   return row;
  },
