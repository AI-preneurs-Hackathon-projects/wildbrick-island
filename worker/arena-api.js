...
export async function handleArenaAPI(request,env,ctx={}){try{
 ...
 const {room}=await store.mutate(session.room_id,(r,t)=>applyInput(r,session.player_id,packet,t,kit));return response({snapshot:roomSnapshot(room,session.player_id)});
