...
## September 14 — jump isolation, join recovery, neutral aim, hammer swing and sky rings

All 169 checks pass. New regressions cover the real Practice Jump button and ArrowUp/Space without actionTime, attack/fire events or damage; independent simultaneous attack+jump remains supported. Arena jumping holds the resting arm pose without attack events. The reported Practice touch bug was a real action-handler coupling: the Jump button called action(), which set the punch animation timer and then jumped. Jump now calls jump() directly, and attack no longer jumps implicitly.

Production Worker logs around the reported failure show canceled join invocations near 12 seconds, using only 0–1 ms CPU, preceded by canceled sync/leave invocations. A separate authenticated browser join subsequently succeeded, so the exact production stall was intermittent. A local regression demonstrates that a stranded room request previously blocked every following same-isolate mutation through the shared promise queue. Mutations now own their request I/O and rely on the existing D1 compare-and-swap for correctness. The request timeout remains 12 seconds. Durable token-proven join retries reuse one seat after a lost response, and rejoin does not await leaving the previous seat. SQLite/client tests cover simultaneous retries, wrong-principal/token rejection, expired-session recovery, cancel-before-room-write, late responses, deliberate rejoin, timeout wording and retry, and cleanup without deleting a newer player. This addresses the demonstrated failure mechanism; it does not prove all external network failures are eliminated.

Weapon tilt UI, R/F aiming and instructions are removed. The authoritative aim helper neutralizes legacy pitch fields while preserving character yaw, right-handed grips and camera-independent projectiles. Camera drag still looks upward. Generated held melee weapons now rotate around local X toward character +Z rather than around Z sideways. A deterministic hammer fixture verifies forward head travel at four character facings and forward melee damage with an unharmed side target. The shared angle helper is used in Practice and Arena. Local WebGL inspection from side and rear showed the hammer head swinging ahead while camera orbit left body direction unchanged. This fixture is not the user's exact generated hammer; no new generation request was made.

Sky-ring radius is 5.6 meters instead of 4.4. Oriented rim segments test mounted width/depth relative to the ring plane, avoiding the previous diagonal-footprint obstruction of the empty center. Other prop blocking remains unchanged. Crossings of the aperture in either direction award Practice progress once per ring; merely hovering nearby or crossing beside the rim does not. Tests pass through all five actual ring orientations from both sides using the starter plane and a saved generated dragon. Local WebGL inspection in the full world confirmed crossing ring 2 from both directions with the plane. The real Practice Jump button showed an airborne builder with no strike; physical touch hardware was not tested.
