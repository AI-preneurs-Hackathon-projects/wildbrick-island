...
import {EQUIPMENT_DROPS} from '../public/supply-catalog.js';
...
const actions={state:()=>state,start(){state.started=true;},openArena(){joins++;},recent:()=>[],saved:()=>true,build(){},action(){fires++;},jump(){jumps++;},voice(){},sound(){},auto(){},pause(v){state.paused=v;input.clear();},respawn(){},cancelDesign(){},leaveArena(){leaves++;state.arena=false;state.started=false;ui.showEntry();}};
...
check('arena entry and health/leaderboard UI bind the joined room and escape names',()=>{document.querySelector('#start').click();assert.equal(joins,1);const r=newRoom(),p=addPlayer(r,'a','<img src=x>');p.health=44;p.kit=makeKit('car');p.mountHealth=100;const client={self:p,snapshot:roomSnapshot(r,p.id),active:true,room:'REAL',serverTime:()=>r.time};ui.update(state);arenaUI.update(client);assert.equal(document.querySelector('#arena-room'),null);assert.equal(document.querySelector('#explore-start'),null);arenaUI.update(client);assert.match(document.querySelector('#objective-title').textContent,/Wildbrick Island/);assert.equal(document.querySelector('#arena-health').textContent,'44');assert.equal(document.querySelector('#arena-ranks img'),null);const board=document.querySelector('#arena-board');assert.equal(board.open,true);assert.equal(board.querySelector('button'),null);board.open=false;arenaUI.update(client);assert.equal(board.open,false);board.open=true;assert.equal(document.querySelector('#action').classList.contains('hidden'),true);p.kit=makeKit();p.health=0;p.respawnAt=r.time+12000;arenaUI.update(client);assert.match(document.querySelector('#arena-respawn').textContent,/12/);arenaUI.error('Sign in',401);assert.equal(document.querySelector('#arena-sign-in').classList.contains('hidden'),false);assert.equal(document.querySelector('#arena-sign-in').target,'_top');document.querySelector('#arena-lobby').close();});
...
check('weapon and rover parachutes show actual models and release owned render resources',()=>{
 const scene=new THREE.Scene(),view=createArenaView(scene,new THREE.PerspectiveCamera()),r=newRoom(),p=addPlayer(r,'p','Collector');
 r.drops=EQUIPMENT_DROPS.map((equipment,i)=>({id:'gear:'+i,type:'equipment',equipment,name:equipment,x:i*5,z:0,born:r.time,lands:r.time+5500,expires:r.time+45000}));view.update(roomSnapshot(r,p.id),p,new Map(),1/60,r.time);
 const drop=scene.getObjectByName('airdrop:gear:2');assert.ok(drop.getObjectByName('supply-model').children.length>0);assert.ok(drop.userData.canopy.visible);assert.match(drop.userData.text,/Landing/);const height=drop.position.y;
 view.update(roomSnapshot(r,p.id),p,new Map(),1/60,r.time+5600);assert.ok(drop.position.y<height);assert.equal(drop.userData.canopy.visible,false);assert.match(drop.userData.text,/Walk onto/);p.kit=makeKit('sword');view.update(roomSnapshot(r,p.id),p,new Map(),1/60,r.time+5700);assert.match(drop.userData.text,/Return to foot/);
 scene.updateMatrixWorld(true);scene.traverse(o=>assert.ok(o.matrixWorld.elements.every(Number.isFinite)));let disposed=0;drop.userData.texture.addEventListener('dispose',()=>disposed++);view.clear();assert.equal(disposed,1);assert.equal(scene.getObjectByName('airdrop:gear:2'),undefined);
});
...
check('Leave arena is available only in the pause menu and returns to entry',()=>{
