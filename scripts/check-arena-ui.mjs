...
const recent=[{name:'Saved moon rover'}],rebuilds=[];const state=createState();state.started=true;let leaves=0,joins=0,fires=0,jumps=0,mics=0,imagines=0,exits=0;const builds=[];
const actions={state:()=>state,start(){state.started=true;},openArena(){joins++;},recent:()=>recent,saved:()=>true,rebuild(i){rebuilds.push(i);},build(){},action(){fires++;},jump(){jumps++;},voice(){},sound(){},auto(){},pause(v){state.paused=v;input.clear();},respawn(){},cancelDesign(){},exitHome(){state.started=false;ui.showEntry();leaves++;state.arena=false;arenaUI.reset();ui.resetPlayUI();ui.update(state);}};
localStorage.setItem('brickwild-tour-v1','seen');
const ui=createUI(actions),arenaUI=createArenaUI({getPlayerName:ui.playerName,join:async()=>true,leave(){},exitHome:actions.exitHome,openCollection:()=>ui.openMenu('collection'),toast(){}}),input=createInput({onBuild:mode=>{builds.push(mode);if(mode==='foot')exits++;},onAction:actions.action,onJump:actions.jump,onPause(){ui.toggleMenu('pause');},onMic(){mics++;},onImagine(){imagines++;ui.openMenu('imagine');},onHotkeys(){ui.toggleMenu('hotkeys');},onHome(){},getState:()=>state});
...
check('R/F pitch only the camera and never enter weapon or movement input',()=>{input.clear();const viewPitch=input.pitch,yaw=input.yaw,before=input.read();key('keydown','KeyR');input.update(.1);assert.ok(input.pitch<viewPitch);assert.deepEqual(input.read(),before);assert.equal(input.read().weaponPitch,0);key('keyup','KeyR');key('keydown','KeyF');input.update(.1);assert.equal(input.pitch,viewPitch);assert.deepEqual(input.read(),before);assert.equal(input.yaw,yaw);assert.equal(document.querySelector('#weapon-tilt-controls'),null);key('keyup','KeyF');for(let i=0;i<20;i++){key('keydown','KeyR');input.update(.1);}assert.equal(input.pitch,-.75);key('keyup','KeyR');for(let i=0;i<20;i++){key('keydown','KeyF');input.update(.1);}assert.equal(input.pitch,.92);key('keyup','KeyF');input.clear();});
check('arena entry and health/leaderboard UI bind the joined room and escape names',()=>{document.querySelector('#start').click();assert.equal(joins,1);const r=newRoom(),p=addPlayer(r,'a','<img src=x>');p.health=44;p.kit=makeKit('car');p.mountHealth=100;const client={self:p,snapshot:roomSnapshot(r,p.id),active:true,room:'REAL',serverTime:()=>r.time};ui.update(state);arenaUI.update(client);assert.equal(document.querySelector('#arena-room'),null);assert.ok(document.querySelector('#explore-start'));arenaUI.update(client);assert.equal(document.body.classList.contains('in-arena'),true);assert.ok(document.querySelector('#practice-route'));assert.equal(document.querySelector('#arena-health').textContent,'44');assert.equal(document.querySelector('#arena-ranks img'),null);const board=document.querySelector('#arena-board');assert.equal(board.open,true);assert.equal(board.querySelector('button'),null);board.open=false;arenaUI.update(client);assert.equal(board.open,false);board.open=true;assert.equal(document.querySelector('#action').classList.contains('hidden'),true);p.kit=makeKit();p.health=0;p.respawnAt=r.time+12000;arenaUI.update(client);assert.match(document.querySelector('#arena-respawn').textContent,/12/);arenaUI.error('Sign in',401);assert.equal(document.querySelector('#arena-sign-in').classList.contains('hidden'),false);assert.equal(document.querySelector('#arena-sign-in').target,'_top');document.querySelector('#arena-lobby').close();});
...
check('a quick pointer or keyboard tap is latched once and its click cannot enqueue a second attack',()=>{
 ...
 action.click();assert.equal(fires,before+1,'keyboard/assistive click keeps its discrete action');input.clear();
});
check('camera drag changes view without changing weapon tilt',()=>{const tilt=input.read().weaponPitch;const scene=document.querySelector('#scene');const pointer=(type,x,y)=>{const e=new window.Event(type);Object.assign(e,{pointerId:8,clientX:x,clientY:y});scene.dispatchEvent(e);};pointer('pointerdown',0,300);pointer('pointermove',0,-300);assert.equal(input.read().weaponPitch,tilt);assert.equal(input.pitch,-.75);pointer('pointermove',0,1000);assert.equal(input.read().weaponPitch,tilt);assert.equal(input.pitch,.92);pointer('pointerup',0,1000);});
...
check('Enter starts once, advances the tour and resumes; text entry and repeat stay safe',()=>{
 ...
 assert.match(document.querySelector('.move-hint').textContent,/R \/ F.*pitch/i);assert.doesNotMatch(document.querySelector('.move-hint').textContent,/weapon tilt/i);
});
check('Your creations opens the existing collection, clears held input and rebuilds once',()=>{
 input.clear();state.arena=true;const r=newRoom(),p=addPlayer(r,'p','Builder'),client={active:true,self:p,snapshot:roomSnapshot(r,p.id),serverTime:()=>r.time};arenaUI.update(client);ui.closeMenu();const before=fires,oldJumps=jumps;
 assert.equal(document.querySelector('#arena-creations').textContent,'Your creations');assert.equal(document.querySelector('#action kbd').textContent,'→');key('keydown','KeyW');key('keydown','ArrowRight');document.querySelector('#arena-creations').click();assert.ok(document.querySelector('#menu').open);assert.ok(state.paused);assert.equal(input.read().fire,false);assert.equal(input.read().z,0);assert.match(document.querySelector('#menu-content').textContent,/Saved moon rover/);key('keydown','ArrowRight');key('keydown','ArrowUp');assert.equal(fires,before);assert.equal(jumps,oldJumps);
 document.querySelector('[data-recent="0"]').click();assert.deepEqual(rebuilds,[0]);assert.equal(document.querySelector('#menu').open,false);assert.equal(state.paused,false);assert.equal(input.read().fire,false);assert.equal(input.read().z,0);assert.equal(recent.length,1);
 ui.openMenu('pause');assert.equal(document.querySelector('#collection'),null);assert.equal(document.querySelector('#tour-again'),null);assert.equal(document.querySelector('#type-fallback'),null);ui.closeMenu();
});
...
check('round countdown, intermission scoreboard and automatic next round stay in Arena',()=>{
 const r=newRoom(),p=addPlayer(r,'p','<script>');p.kills=5;p.deaths=2;let restarts=0;const client={self:p,snapshot:roomSnapshot(r,p.id),active:true,serverTime:()=>r.time};arenaUI.setStatus('online');arenaUI.update(client);assert.match(document.querySelector('#arena-round-clock').textContent,/5:00/);p.lastSeen=r.round.endsAt;advanceRoom(r,r.round.endsAt);client.snapshot=roomSnapshot(r,p.id);arenaUI.update(client);assert.equal(document.querySelector('#arena-results').classList.contains('hidden'),false);assert.equal(document.querySelector('#arena-final-ranks script'),null);assert.equal(document.querySelector('#action').classList.contains('hidden'),true);assert.equal(document.querySelector('#mic').disabled,true);assert.equal(document.querySelector('#arena-restart'),null);assert.match(document.querySelector('#arena-intermission').textContent,/30 seconds/);p.lastSeen=r.round.intermissionEndsAt;advanceRoom(r,r.round.intermissionEndsAt);client.snapshot=roomSnapshot(r,p.id);arenaUI.update(client);assert.match(document.querySelector('#arena-round-clock').textContent,/Round 2.*5:00/);assert.equal(document.querySelector('#arena-results').classList.contains('hidden'),true);assert.equal(document.querySelector('#arena-previous-ranks'),null);assert.equal(document.querySelector('#mic').disabled,false);assert.equal(client.self.id,'p');
});
check('Arena HUD hides range/interval, keeps connection errors, and final-scoreboard Exit goes home',()=>{
 const r=newRoom(),p=addPlayer(r,'p','Builder');p.kit=makeKit('bow');p.buildReadyAt=r.time+12000;const client={self:p,snapshot:roomSnapshot(r,p.id),active:true,serverTime:()=>r.time};
 arenaUI.setStatus('online');arenaUI.update(client);assert.equal(document.querySelector('#arena-round-clock').textContent,'Round 1 · 5:00');assert.equal(document.querySelector('#arena-stats'),null);assert.equal(document.querySelector('#mode-pill span').textContent,p.kit.name);assert.equal(document.querySelector('#action span').textContent,'Fire');assert.equal(document.querySelector('#arena-network').textContent,'');assert.ok(document.querySelector('#arena-network').classList.contains('hidden'));assert.equal(p.buildReadyAt,r.time+12000);
 arenaUI.setStatus('reconnecting');arenaUI.update(client);assert.match(document.querySelector('#arena-network').textContent,/Reconnecting/);assert.equal(document.querySelector('#arena-network').classList.contains('hidden'),false);
 p.lastSeen=r.round.endsAt;advanceRoom(r,r.round.endsAt);client.snapshot=roomSnapshot(r,p.id);arenaUI.update(client);assert.equal(document.querySelector('#arena-round-clock').textContent,'Round 1 · Complete');assert.equal(document.querySelector('#arena-results').lastElementChild.id,'arena-exit-home');assert.equal(document.querySelector('#arena-restart'),null);const before=leaves;document.querySelector('#arena-exit-home').click();assert.equal(leaves,before+1);assert.equal(state.started,false);assert.equal(state.arena,false);assert.ok(document.querySelector('#arena-results').classList.contains('hidden'));assert.equal(document.querySelector('#intro').classList.contains('hidden'),false);assert.ok(document.querySelector('#player-name'));assert.ok(document.querySelector('#explore-start'));assert.ok(document.querySelector('#start'));
});
check('expired arena keeps vitals, exposes Rejoin, and hides Lower while the player is on foot',()=>{state.started=true;
 ...
 assert.equal(document.querySelector('#arena-health').textContent,'76');assert.equal(document.querySelector('#mode-pill span').textContent,'On foot');assert.match(document.querySelector('#arena-network').textContent,/position held/);assert.equal(document.querySelector('#open-arena').textContent,'Rejoin');assert.equal(document.querySelector('#open-arena').style.display,'inline-flex');
