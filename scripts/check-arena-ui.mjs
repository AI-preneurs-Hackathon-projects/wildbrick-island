...
check('Previous creations opens the existing collection, clears held input and rebuilds once',()=>{
 ...
 ui.openMenu('pause');assert.equal(document.querySelector('#collection'),null);assert.ok(document.querySelector('#tour-again'));assert.equal(document.querySelector('#type-fallback'),null);ui.closeMenu();
});
