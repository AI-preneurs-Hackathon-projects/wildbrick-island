...
check('R/F has no aiming effect and no weapon tilt controls remain',()=>{input.clear();const viewPitch=input.pitch,yaw=input.yaw;for(const code of ['KeyR','KeyF']){key('keydown',code);input.update(.1);key('keyup',code);assert.equal(input.read().weaponPitch,0);}assert.equal(input.pitch,viewPitch);assert.equal(input.yaw,yaw);assert.equal(document.querySelector('#weapon-tilt-controls'),null);input.clear();});
...
check('Enter starts once, advances the tour and resumes; text entry and repeat stay safe',()=>{
 ...
 assert.doesNotMatch(document.querySelector('.move-hint').textContent,/R \/ F|weapon tilt/i);
});
...
check('Arena HUD keeps range/interval and connection errors; final-scoreboard Exit goes home',()=>{
 ...
 arenaUI.setStatus('online');arenaUI.update(client);assert.equal(document.querySelector('#arena-round-clock').textContent,'Round 1 · 5:00');assert.equal(document.querySelector('#arena-stats').textContent,`${p.kit.stats.range}m · ${p.kit.stats.interval}s`);assert.equal(document.querySelector('#mode-pill span').textContent,p.kit.name);assert.equal(document.querySelector('#action span').textContent,'Fire');assert.equal(document.querySelector('#arena-network').textContent,'');assert.ok(document.querySelector('#arena-network').classList.contains('hidden'));assert.equal(p.buildReadyAt,r.time+12000);
...
check('expired arena keeps vitals, exposes Rejoin, and retains a touch Lower control after dismount',()=>{state.started=true;
