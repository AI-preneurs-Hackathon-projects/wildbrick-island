...
for(const [file,weapon,movement] of [['final-compact-none/dragon','flame','fly'],['final-compact-none/armed-car','automatic','drive'],['final-compact-none/octopus','pulse','drive']]){
 ...
 const room=newRoom(100000);for(const e of WORLD_ENTITIES)room.destroyed[e.id]=1e12;const p=addPlayer(room,'builder','Builder');Object.assign(p,{x:0,z:10,protectedUntil:0});
 ...
 applyInput(room,p.id,{seq:2,input:{cameraYaw:0},command:{id:2,type:'fire'}},room.time);advanceRoom(room,room.time+500);assert.ok(victim.health<100,'authored emitter launches a working trusted attack');assert.ok(room.events.some(e=>e.type==='hit'&&e.by===p.id));assert.equal(p.x,0,'attack never knocks the shooter sideways');assert.equal(p.z,10,'attack never knocks the shooter forward');if(movement==='fly')assert.equal(p.y,9);else assert.ok(p.y<9,'ground mounts fall when unsupported');
