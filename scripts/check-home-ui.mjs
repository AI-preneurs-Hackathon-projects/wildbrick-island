...
function setup(joinImpl=async()=>true){
 const state=createState(),joins=[];let starts=0,leaves=0,resets=0,snapshots=0,designCancels=0,arenaUI,entry;
 ...
 const ui=createUI({state:()=>state,snapshot(){snapshots++;},cancelDesign(){designCancels++;},start(){state.started=true;starts++;},openArena(){return arenaUI.play();},resetPractice(){resets++;for(const key of ['gates','targets','crates','rings'])state[key]=[];state.bricks=0;state.won=false;ui.update(state);},pause(v){state.paused=v;},recent:()=>[],saved:()=>true,exitHome,leaveArena:exit});
 ...
 return {ui,arenaUI,state,joins,exit,get starts(){return starts;},get leaves(){return leaves;},get resets(){return resets;},get snapshots(){return snapshots;},get designCancels(){return designCancels;},get entry(){return entry;}};
}
...
await check('both modes require a name and recover from whitespace input',async()=>{
 const app=setup();assert.equal(document.querySelector('.intro-small'),null);assert.doesNotMatch($('intro').textContent,/Keyboard or touch|Type or speak to build/);assert.equal($('hud').classList.contains('hidden'),true);assert.equal($('hud').getAttribute('aria-hidden'),'true');for(const id of ['practice-map-name','snapshot','help','pause','mode-pill','mic','action'])assert.equal($(id).closest('#hud'),$('hud'));assert.equal($('jump-equipped'),null);assert.equal($('snapshot').getAttribute('aria-label'),'Take a snapshot');assert.equal($('snapshot').getAttribute('aria-keyshortcuts'),'0');assert.equal($('cancel-design'),null);app.ui.generationState({status:'designing',prompt:'test idea'});assert.equal(document.querySelector('.generation-cancel-hint').textContent,'Press X to cancel');window.dispatchEvent(new window.KeyboardEvent('keydown',{code:'KeyX',key:'x',bubbles:true}));assert.equal(app.designCancels,1);assert.equal($('generation').classList.contains('hidden'),true);app.ui.designError('Other builders are busy.');assert.equal(document.querySelector('.generation-cancel-hint').textContent,'Press X to close');window.dispatchEvent(new window.KeyboardEvent('keydown',{code:'KeyX',key:'x',bubbles:true}));assert.equal(app.designCancels,2);assert.equal($('generation').classList.contains('hidden'),true);app.ui.toast('First sentence. Second sentence!');assert.equal($('toast').textContent,'First sentence.\nSecond sentence!');for(const id of ['explore-start','start']){$(id).click();assert.equal(app.starts,0);}
...
await check('Explore keeps newer speech controls and shows the full Practice route without Arena or rewards',async()=>{
 ...
 app.state.speed=2;app.ui.update(app.state);assert.equal(document.body.classList.contains('hud-in-motion'),false,'movement dimming defaults off');$('hud-dim').click();assert.equal(document.body.classList.contains('hud-in-motion'),true);app.state.speed=0;app.ui.update(app.state);assert.equal(document.body.classList.contains('hud-in-motion'),false);
...
await check('first-play tour respects Explore choice and does not join Arena on completion',()=>{
 localStorage.removeItem('brickwild-tour-v1');const app=setup();name('River');$('explore-start').click();assert.equal($('tutorial').open,true);$('tour-next').click();assert.match($('tutorial').textContent,/Imagine it/);$('tour-next').click();assert.match($('tutorial').textContent,/Use your creations/);$('tour-next').click();assert.match($('tutorial').textContent,/Tips for useful builds/);assert.match($('tutorial').textContent,/car I can drive/);assert.match($('tutorial').textContent,/dragon I can ride and fly with fire breath/);assert.match($('tutorial').textContent,/may have no action/);$('tour-next').click();assert.match($('tutorial').textContent,/Follow the Practice route/);for(const label of ['Scenic route','Right on target','Smash & grab','Sky is the limit'])assert.match($('tutorial').textContent,new RegExp(label.replace('&','&')));assert.match($('tutorial').textContent,/Health supplies collect automatically/);$('tour-next').click();assert.equal(app.joins.length,0);assert.equal(app.state.paused,false);
});
