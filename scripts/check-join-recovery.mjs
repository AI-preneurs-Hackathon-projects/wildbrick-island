...
await check('concurrent retries and a lost join response reuse one authenticated seat',async()=>{
 const resume=ticket(),body={resume,room:'RETRY',name:'Retry',motionVersion:1};const responses=await Promise.all([api('/api/arena/join',body),api('/api/arena/join',body)]);assert.ok(responses.every(r=>r.status===200));const [a,b]=await Promise.all(responses.map(r=>r.json()));assert.equal(a.session,b.session);assert.equal(a.snapshot.self,b.snapshot.self);assert.equal(count('RETRY'),1);
 ...
 assert.equal((await api('/api/arena/join',body,'someone-else')).status,401);assert.equal((await api('/api/arena/join',{...body,resume:{...resume,token:'a'.repeat(64)}})).status,401);
...
await check('leaving during a pending room join prevents a late seat from reappearing',async()=>{
 ...
 const req=new Request('https://brickwild.test/api/arena/join',{method:'POST',headers:{'Content-Type':'application/json',Origin:'https://brickwild.test','oai-authenticated-user-id':'recovery'},body:JSON.stringify({resume,room:'RACE',name:'Canceled'})});
...
function clientHarness(room){const timers=new Map(),requests=[],errors=[];let next=0,mode='ok',release=null;
 ...
 return {client,requests,errors,timers,setMode:m=>mode=m,release:()=>release?.(),get ready(){return !!release;},join:()=>client.join('Tester',room)};
}
