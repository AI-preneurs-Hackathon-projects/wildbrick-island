...
try{
 ...
 $('voice-reset').click();$('voice-delay').checked=true;$('mic').click();$('record-voice').click();await flush();$('mic').click();await run(100);window.dispatchEvent(new window.Event('pagehide'));await run(5000);assert.match($('voice-review-status').textContent,/creation callbacks: 0/);
