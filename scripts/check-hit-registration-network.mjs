...
for(const rtt of [50,150,300]){
 ...
 function peer(user){
  ...
  const client=createArenaClient({onEvent:e=>{events.push(e);view.effect(e);},onError:e=>errors.push(e)},{fetcher:async(path,options)=>{
   await wait(rtt/2);const body=JSON.parse(options.body||'{}');if(body.command?.type==='fire')fireRequests++;
 ...
 try{
  ...
  for(const weapon of ['bow','foot']){
   await store.mutate('TEST',(r,t)=>{for(const e of WORLD_ENTITIES)r.destroyed[e.id]=1e12;r.nextDrop=t+1e9;for(const [id,z] of [[shooter,0],[target,weapon==='bow'?.9:0]]){const p=r.players[id];Object.assign(p,{x:0,y:0,z,yaw:0,health:100,mountHealth:0,kit:makeKit(id===shooter?weapon:'foot'),protectedUntil:0,nextShot:0,input:{},melee:null});p.spawnSerial=(p.spawnSerial||0)+1;p.motion=newMotion(t);}});
   ...
   const before=read().eventId;conflicts=1;a.lose();const previews=a.events.filter(e=>e.type==='attack-preview').length;
   ...
   assert.equal(room.players[target].health,weapon==='bow'?78.4:83.8);assert.equal(room.players[shooter].health,100);
   ...
   assert.equal(read().events.filter(e=>e.id>before&&e.type==='hit').length,1);assert.equal(a.dropped,weapon==='bow'?1:2);assert.equal(conflicts,0);
  ...
  assert.ok(a.fireRequests>=4,'both committed fire commands retried');assert.deepEqual(a.errors,[]);assert.deepEqual(b.errors,[]);
