...
export async function runScenario(scenario,implementation={}){return randomSeed(async()=>{
 ...
 for(const s of n.stats){assert.equal(s.inFlight,0);assert.equal(s.syncInFlight,0);assert.ok(s.pendingMax<=CRITERIA.pendingFramesMaximum);assert.equal(s.duplicateShots,0);assert.equal(s.duplicateCommandShots,0);assert.equal(s.authoritativeDuplicateCommandShots,0);assert.equal(s.commandOverlap,0);assert.ok(s.movementReplayErrorM<1e-6,'accepted movement differs from shared movement replay');assert.ok(s.driftMaxM<=90/60*.6*(implementation.core||defaultCore).makeKit('foot').stats.speed+1e-6,'fixed 0.6-axis fixture pose gap exceeds its 90-frame travel bound');assert.ok(s.acceptedFrames<=(scenario.duration+10000)/1000*60+CRITERIA.authoritativeFrameSlack);}
