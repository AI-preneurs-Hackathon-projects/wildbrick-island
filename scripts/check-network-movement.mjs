...
await check('a rebuilt legacy rover survives delayed snapshots without relocating or fetching a new blueprint',async()=>{
 const n=network(600);await n.join();for(let i=0;i<65;i++)await n.step();const before=pose(n.client.self);n.player.building={kit:makeKit('supply:rover',n.client.blueprints.get('supply:rover')),starts:n.room.time,ends:n.room.time+200};
 for(let i=0;i<120;i++)await n.step();assert.equal(n.client.self.kit.id,'supply:rover');assert.deepEqual(pose(n.client.self),before);assert.equal(n.packets.filter(p=>p.path.includes('/blueprint')).length,0);assert.equal(n.events.filter(e=>e.type==='built').length,1);assert.equal(n.client.blueprints.get('supply:rover').movement,'drive');
});
...
await check('local and authoritative firing ignore camera orbit, including immediate preview',async()=>{
 const n=network(120);await n.join();Object.assign(n.player,{kit:makeKit('bow'),protectedUntil:0,yaw:.8});await n.advance(500);const expected=solveWeaponAim(n.client.snapshot,n.client.self);await n.step({fire:true,cameraYaw:-2.4,aimPitch:.75,weaponPitch:.3});const preview=n.events.find(e=>e.type==='attack-preview');assert.ok(preview);assert.equal(preview.yaw,expected.yaw);assert.ok(Math.abs(preview.yaw-.8)<=HANDHELD_AIM_CAP);assert.equal(preview.pitch,0);await n.advance(500);const shot=n.room.events.find(e=>e.type==='shot');assert.ok(shot);assert.ok(Math.abs(shot.aimYaw-solveWeaponAim(n.room,n.player).yaw)<1e-7);assert.ok(Math.abs(shot.yaw-shot.aimYaw)<=n.player.kit.stats.spread/2);assert.equal(n.player.yaw,.8);n.client.leave();
});
await check('automatic intermission stays joined through lost responses and starts the next round once',async()=>{
 const n=network(100);await n.join();const id=n.client.snapshot.self;n.player.kills=4;n.room.round.endsAt=n.now+100;await n.advance(500);assert.equal(n.client.snapshot.round.status,'finished');assert.equal(n.client.command('fire'),false);assert.equal(n.client.command('restart'),false);n.loseResponse();await n.advance(1800);assert.equal(n.client.snapshot.round.id,1);await n.advance(15000);assert.equal(n.client.snapshot.round.id,2);assert.equal(n.client.snapshot.self,id);assert.equal(n.player.kills,0);assert.equal(n.client.snapshot.round.previousResults,undefined);assert.equal(n.room.events.filter(e=>e.type==='round-started').length,1);assert.equal(n.packets.filter(p=>p.path.endsWith('/join')).length,1);assert.equal(n.packets.filter(p=>p.path.endsWith('/leave')).length,0);n.client.leave();
});
