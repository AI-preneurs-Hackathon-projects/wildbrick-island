...
await check('local and authoritative firing ignore camera orbit, including immediate preview',async()=>{
 const n=network(120);await n.join();Object.assign(n.player,{kit:makeKit('bow'),protectedUntil:0,yaw:.8});await n.advance(500);await n.step({fire:true,cameraYaw:-2.4,aimPitch:.75,weaponPitch:.3});const preview=n.events.find(e=>e.type==='attack-preview');assert.ok(preview);assert.equal(preview.yaw,.8);assert.equal(preview.pitch,0);await n.advance(500);const shot=n.room.events.find(e=>e.type==='shot');assert.ok(shot);assert.ok(Math.abs(shot.yaw-.8)<.02);assert.equal(n.player.yaw,.8);n.client.leave();
});
