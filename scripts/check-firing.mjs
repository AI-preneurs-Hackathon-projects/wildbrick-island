...
await check('every ranged weapon preserves body facing while aiming horizontally and vertically',()=>{
 for(const weapon of ['bow','pulse','automatic','flame'])for(const movement of ['carry','drive','fly'])for(const yaw of [-2.1,0,1.7])for(const pitch of [-.65,0,.65]){
  ...
  applyInput(r,p.id,{seq:1,input:{cameraYaw:yaw,aimPitch:pitch},command:{id:1,type:'fire'}},r.time);
  near(p.yaw,1.2);const b=r.projectiles[0],event=r.events.find(e=>e.type==='shot'),from=weaponMuzzle(p,yaw,pitch);assert.ok(b);assert.deepEqual(event.projectile,b);
  for(const k of ['x','y','z'])near(b[k],from[k]);near(Math.atan2(b.vx,b.vz),yaw);near(Math.atan2(b.vy,Math.hypot(b.vx,b.vz)),pitch);
 }
});
await check('ground, target and world-cover contacts retain the shot ID and exact position',()=>{
 for(const kind of ['ground','target','cover']){
  const {r,p}=setup();p.kit.muzzle=[0,1.4,0];const pitch=kind==='ground'?-.65:0;
  if(kind!=='ground')r.placed.push({id:kind,x:0,z:4,w:3,h:20,d:.4,hp:100});
  applyInput(r,p.id,{seq:1,input:{cameraYaw:0,aimPitch:pitch},command:{id:1,type:'fire'}},r.time);const b={...r.projectiles[0]};advanceRoom(r,r.time+1000);
...
await check('a downward carried emitter cannot launch beneath the ground',()=>{
 const {r,p}=setup();p.y=0;p.kit.muzzle=[0,.2,5];applyInput(r,p.id,{seq:1,input:{cameraYaw:0,aimPitch:-.75},command:{id:1,type:'fire'}},r.time);assert.ok(r.projectiles[0].y>=.019);advanceRoom(r,r.time+100);const impact=r.events.find(e=>e.type==='impact');assert.equal(impact.entity,'ground');near(impact.y,0);
});
await check('the island target receives impact feedback on its front surface',()=>{
 const {r,p}=setup(),entity=WORLD_ENTITIES.find(e=>e.id==='target:0'),face=entity.boxes[0];delete r.destroyed[entity.id];Object.assign(p,{x:face.x,y:0,z:face.z-6});applyInput(r,p.id,{seq:1,input:{cameraYaw:0,aimPitch:0},command:{id:1,type:'fire'}},r.time);advanceRoom(r,r.time+500);const impact=r.events.find(e=>e.type==='impact');assert.equal(impact.entity,'target:0');near(impact.z,face.z-face.d/2);assert.ok(r.damage['target:0']>0);
});
...
await check('rendered ranged attacks keep actors facing and align a carried emitter with its shot origin',()=>{
 for(const movement of ['carry','drive','fly']){const blueprint={...fixture,movement},kit=makeKit('test',blueprint),{r,p}=setup(kit),scene=new THREE.Scene(),view=createArenaView(scene,new THREE.PerspectiveCamera()),cache=new Map([['test',blueprint]]);
  ...
  if(movement==='carry'){const model=actor.getObjectByName('creation');scene.updateMatrixWorld(true);const grip=new THREE.Vector3(.76,1.1,.3).applyAxisAngle(new THREE.Vector3(0,1,0),p.yaw).add(actor.position);const expected=weaponMuzzle(p,p.aimYaw,p.aimPitch);const offset=new THREE.Vector3(...kit.muzzle).sub(new THREE.Vector3(.76,1.1,.3)).applyEuler(new THREE.Euler(-p.aimPitch,p.aimYaw,0,'YXZ')).add(grip);near(offset.x,expected.x);near(offset.y,expected.y);near(offset.z,expected.z);near(model.rotation.y,p.aimYaw-p.yaw);}
  view.clear();
 }
});
...
await check('Explore shots obey explicit aim, preserve facing, and stop at the actual wall or ground',async()=>{
 const events=[],sim=await createSimulation([{id:'wall',x:0,y:2,z:4,w:4,h:4,d:.4}],e=>events.push(e));Object.assign(sim.state,{started:true,x:0,y:0,z:0,yaw:1.2});sim.build('bow');for(let i=0;i<120;i++)sim.update(1/60,{});
 sim.action({cameraYaw:0,aimPitch:0});const shot=events.findLast(e=>e.type==='shoot');assert.ok(shot.impact);near(shot.to.z,3.8);near(sim.state.yaw,1.2);assert.equal(shot.target,null);
 sim.update(1,{});sim.state.cooldown=0;sim.action({cameraYaw:Math.PI,aimPitch:-.65});const down=events.findLast(e=>e.type==='shoot');near(down.to.y,0);assert.ok(down.impact);assert.ok(down.to.z<down.from.z);sim.dispose();
});
