...
await check('every ranged weapon follows the character despite sideways and backward camera orbit',()=>{
 for(const weapon of ['bow','pulse','automatic','flame'])for(const movement of ['carry','drive','fly'])for(const yaw of [-2.1,0,1.7])for(const pitch of [-.65,0,.65]){
  ...
  for(const k of ['x','y','z'])near(b[k],from[k]);near(Math.atan2(b.vx,b.vz),p.yaw);near(Math.atan2(b.vy,Math.hypot(b.vx,b.vz)),movement==='carry'?pitch:0);
 }
});
...
await check('a downward carried emitter cannot launch beneath the ground',()=>{
 const {r,p}=setup();p.y=0;p.kit.muzzle=[0,.2,5];applyInput(r,p.id,{seq:1,input:{cameraYaw:0,weaponPitch:-.75},command:{id:1,type:'fire'}},r.time);assert.ok(r.projectiles[0].y>=.019);advanceRoom(r,r.time+100);const impact=r.events.find(e=>e.type==='impact');assert.equal(impact.entity,'ground');near(impact.y,0);
});
...
await check('rendered ranged attacks keep actors facing and align a carried emitter with its shot origin',()=>{
 for(const movement of ['carry','drive','fly']){const blueprint={...fixture,movement},kit=makeKit('test',blueprint),{r,p}=setup(kit),scene=new THREE.Scene(),view=createArenaView(scene,new THREE.PerspectiveCamera()),cache=new Map([['test',blueprint]]);
  ...
  if(movement==='carry'){const model=actor.getObjectByName('creation');scene.updateMatrixWorld(true);const grip=new THREE.Vector3(...weaponGrip(kit)).applyAxisAngle(new THREE.Vector3(0,1,0),p.yaw).add(actor.position);const expected=weaponMuzzle(p,p.aimYaw,p.aimPitch);const offset=new THREE.Vector3(...kit.muzzle).sub(new THREE.Vector3(.76,1.1,.3)).applyEuler(new THREE.Euler(-p.aimPitch,p.yaw,0,'YXZ')).add(grip);near(offset.x,expected.x);near(offset.y,expected.y);near(offset.z,expected.z);near(model.rotation.y,0);}
  view.clear();
 }
});
...
await check('Explore shots ignore camera orbit, retain independent weapon tilt and stop at actual contacts',async()=>{
 ...
 sim.update(1,{});sim.state.cooldown=0;sim.action({cameraYaw:Math.PI,weaponPitch:-.65});const down=events.findLast(e=>e.type==='shoot');near(down.to.y,0);assert.ok(down.impact);assert.ok(down.to.z>down.from.z);sim.dispose();
});
