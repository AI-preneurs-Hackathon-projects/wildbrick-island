...
try{
 for(const enabled of [false,true]){
  ...
  const result=await page.evaluate(async enabled=>{
   ...
   const view=createArenaView(scene,camera),events={};
   const n=performanceFixture({latency:100,diagnostics:enabled?ds:[],onEvent:(index,e)=>{if(index===0){view.effect(e);events[e.type]=(events[e.type]||0)+1;}}});await n.start();n.room.players[n.clients[1].snapshot.self].x=6;await n.advance(220);
   const stop=browserCapture.observeBrowser(window,renderer.domElement);
   for(let frame=0;frame<480;frame++){
    await new Promise(requestAnimationFrame);if(frame===60)n.clients.forEach(c=>c.command('build','bow'));const z=Math.floor(frame/60)%2?-.6:.6;await n.step([{z,fire:frame%60===1},{z,fire:frame%60===1}]);
  ...
  await page.screenshot({path:output+'/'+(enabled?'enabled':'disabled')+'.png'});result.resourcesAfterLeave=await page.evaluate(()=>window.finishFixture());assert.equal(result.resourcesAfterLeave.timers,0);assert.ok(result.events.swing>0);assert.ok(result.events.shot>0);results.push(result);await page.close();
 }
 assert.deepEqual(errors,[]);assert.deepEqual(failures,[]);
}finally{await browser.close();await fs.writeFile(output+'/results.json',JSON.stringify({scope:'Sequential desktop 1440×900 SwiftShader synthetic scene; 480 frames, two moving/firing clients, in-process transport. Not a hardware or hosted benchmark.',results,errors,failures},null,2)+'\n');}
