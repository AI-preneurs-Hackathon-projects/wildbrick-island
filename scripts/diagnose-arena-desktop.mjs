...
try{
 ...
 for(const run of runs){
  ...
  const result=await page.evaluate(async ({enabled,run})=>{
   ...
   const {weaponMuzzle}=await import('/weapon-aim.js'),{solveWeaponAim}=await import('/aiming.js'),core=await import('/arena-core.js');
   const n=performanceFixture({latency:run.latency,diagnostics:enabled?ds:[],setupPlayer:(i,p)=>{if(run.tradeoff)Object.assign(p,{x:i?1:-1,z:10,kit:core.makeKit('bow')});},onEvent:(index,e)=>{if(index===0){view.effect(e);events[e.type]=(events[e.type]||0)+1;if(heldPhase&&e.type==='shot')heldPhaseShots++;}}});await n.start();if(!run.tradeoff)n.room.players[n.clients[1].snapshot.self].x=6;await n.advance(220);
   const stop=browserCapture.observeBrowser(window,renderer.domElement);
   const frames=run.tradeoff?1680:run.heldFire?1320:960;for(let frame=0;frame<frames;frame++){
    await new Promise(requestAnimationFrame);if(frame===480&&!run.tradeoff)n.clients.forEach(c=>c.command('build','bow'));heldPhase=run.heldFire&&frame>=960&&frame<1260;const z=frame<900||heldPhase?(Math.floor(frame/60)%2?-.6:.6):0,fire=heldPhase||frame>600&&frame<900&&frame%60===1;if(run.tradeoff&&frame===720){const p=n.room.players[n.clients[0].snapshot.self];p.protectedUntil=0;core.hurt(n.room,p,1000,null);}const phase=(frame-240)%240;await n.step(run.tradeoff?[0,1].map(i=>frame<240?{}:{x:(i?1:-1)*.35*(phase<120?1:-1),fire:phase<180}):[{z,fire},{z,fire}]);
    ...
    if(run.tradeoff){for(const id of n.clients[0].snapshot.players.map(p=>p.id)){const actor=scene.getObjectByName('arena-player:'+id),flash=actor?.getObjectByName('muzzle-flash'),model=actor?.getObjectByName('creation');if(flash?.visible){if(!model)throw Error('Visible flash has no equipped model');const emitter=new THREE.Vector3(.13,0,.84).applyMatrix4(model.matrixWorld);maxAttachmentError=Math.max(maxAttachmentError,emitter.distanceTo(flash.getWorldPosition(new THREE.Vector3())));attachmentSamples++;}}}else if(flash?.visible){const aim=solveWeaponAim(n.clients[0].snapshot,p),expected=weaponMuzzle({kit:p.kit,x:0,y:0,z:0,yaw:0},{yaw:aim.correction});maxAttachmentError=Math.max(maxAttachmentError,Math.hypot(flash.position.x-expected.x,flash.position.y-expected.y,flash.position.z-expected.z));attachmentSamples++;if(heldPhase)heldPhaseAttachmentSamples++;}
   ...
   if(run.tradeoff){if(!(n.room.players[n.clients[0].snapshot.self].spawnSerial>=1))throw Error('Rendered fixture did not respawn');if(!events.respawn)throw Error('Respawn event was not delivered');}const result={...run,enabled,frames,heldPhaseShots,heldPhaseAttachmentSamples,attachmentSamples,maxAttachmentError,viewport:[innerWidth,innerHeight],renderer:ext?gl.getParameter(ext.UNMASKED_RENDERER_WEBGL):'unavailable',browser:browserCapture.export(),clockDomains:{browser:'performance.now milliseconds',clients:'synthetic fixture milliseconds; no UTC mapping'},diagnostics:ds.map(d=>d.export()),events,stats:n.stats};
