...
try{
 ...
 for(const run of runs){
  const enabled=run.version!=='disabled';
  ...
  const result=await page.evaluate(async ({enabled,run})=>{
   ...
   const view=createArenaView(scene,camera),events={};let attachmentSamples=0,maxAttachmentError=0;
   const {weaponMuzzle}=await import('/weapon-aim.js'),{solveWeaponAim}=await import('/aiming.js');
   const n=performanceFixture({latency:run.latency,diagnostics:enabled?ds:[],onEvent:(index,e)=>{if(index===0){view.effect(e);events[e.type]=(events[e.type]||0)+1;}}});await n.start();n.room.players[n.clients[1].snapshot.self].x=6;await n.advance(220);
   const stop=browserCapture.observeBrowser(window,renderer.domElement);
   for(let frame=0;frame<960;frame++){
    await new Promise(requestAnimationFrame);if(frame===480)n.clients.forEach(c=>c.command('build','bow'));const z=frame<900?(Math.floor(frame/60)%2?-.6:.6):0;await n.step([{z,fire:frame>600&&frame<900&&frame%60===1},{z,fire:frame>600&&frame<900&&frame%60===1}]);
    ...
    if(flash?.visible){const aim=solveWeaponAim(n.clients[0].snapshot,p),expected=weaponMuzzle({kit:p.kit,x:0,y:0,z:0,yaw:0},{yaw:aim.correction});maxAttachmentError=Math.max(maxAttachmentError,Math.hypot(flash.position.x-expected.x,flash.position.y-expected.y,flash.position.z-expected.z));attachmentSamples++;}
...
}finally{await browser.close();await fs.writeFile(output+'/results.json',JSON.stringify({scope:'Sequential desktop 1440×900 SwiftShader synthetic scene; 960 frames, two moving/firing clients, in-process transport. Not a hardware or hosted benchmark.',results,errors,failures},null,2)+'\n');}
