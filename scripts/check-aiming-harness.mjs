...
set('weapon','dragon');el('enable-input').checked=true;el('enable-input').onchange();click('lift-shooter');const key=code=>{window.dispatchEvent(new window.KeyboardEvent('keydown',{code}));},up=code=>window.dispatchEvent(new window.KeyboardEvent('keyup',{code}));key('ArrowDown');click('advance');assert.match(result().text,/Shooter height \/ kit: 5\.20 \/ harness/);key('KeyG');up('KeyG');assert.match(result().text,/Recoverable items: 1/);up('ArrowDown');for(let i=0;i<12;i++)click('advance');assert.match(result().text,/Shooter height \/ kit: 0\.00 \/ foot/);key('KeyT');up('KeyT');assert.match(result().text,/Recoverable items: 0/);assert.match(result().text,/Shooter height \/ kit: 0\.00 \/ harness/);
console.log('PASS fixture route, 28 original harness cases, 2 presentation cases and 4 control cases: fixed/converged pulse at 2/5/10/20m, cap saturation, mirrored origin, both cover paths, protection, focus transitions, camera orbit, legacy rebuild and mounted fixed barrels. No-op renderer: NO pixels or hosted multiplayer evidence.');
