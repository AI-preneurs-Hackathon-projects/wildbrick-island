...
await check('restored baseline is characterized without claiming an improvement',async()=>{
 const result=await runScenario(scenarios.find(s=>s.name==='hold-1600'));for(const s of result.peers){assert.equal(s.zeroMovement,369);assert.equal(s.pendingMax,90);assert.ok(s.maxSyncInFlight<=1);assert.equal(s.releaseOvershootM,0);}
