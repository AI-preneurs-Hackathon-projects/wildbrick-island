...
Generation now reports request, upstream, validation and client model-construction timing. A received local blueprint is reused for the arena upload acknowledgement instead of fetching it again. Assembly takes 1.2 seconds (previously 2.8) and preserves movement. A compact profile is available for measured trials via server `OPENAI_BLUEPRINT_DETAIL=compact`: version 4 allows 20–64 parts and requests 20–40, while saved versions 1–3 retain their original validation and joint rules. The model remains a server runtime setting; no client model override exists.

D1 generation admission now shares four expiring upstream slots, 30 starts/minute for the Site and eight starts/minute per trusted requester, in addition to isolate admission and body/time limits. These are resource/billing protections, not player seat limits. Migration `0002_robust_silver_samurai.sql` must be applied with the build. Requests fail with a useful retry message if durable admission is unavailable. API account spend limits remain the final billing boundary.
