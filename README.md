...
Arena rounds last five minutes from the first join, with one server deadline shared by all clients. At the deadline, combat stops and the final KO/fall scoreboard is frozen, including departed participants. Any player can restart inside Arena. Concurrent or retried restart commands for the same round only advance it once. Restart clears scores, effects and projectiles, restores health, and preserves sessions, avatars and equipped creations. The preceding results remain in Last round. Old movement and action packets cannot affect a later round. Existing rooms acquire a deadline when first loaded by this release. No weapon or vehicle lifetimes are introduced.
