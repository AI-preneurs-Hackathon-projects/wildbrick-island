...
**Choose Practice or Arena → speak an idea → watch it build → explore or battle.**
...
Enter a builder name on the home screen and choose **Practice** for the existing solo challenges or **Arena** for the shared **ISLAND**. The name is limited to 20 characters and retained for the browser tab through refresh/sign-in when session storage is available. Enter in the name field chooses Practice. Arena preserves the latest speech, combat, supplies and generation systems, without a room-code form, eight-player seat limit or automatic room splitting. Site sign-in and viewer access still apply. The pause menu offers **Exit to home** in either mode. It clears Arena state immediately, even offline, and returns to the builder-name screen. The join/reconnect dialog retains Back to Practice. A transparent 3D builder preview floats on the right of the home screen: drag or use left/right arrows to rotate, and select one of eight shirt colors. The color is saved on this device when storage is available, used in Practice, and shared with other Arena players on joining.
...
| Look / aim | Q/E horizontally; drag the world to look in any direction | Drag the world |
...
Text entry, browser modifiers, focus loss and dialogs suppress gameplay shortcuts. The multiplayer match continues while a menu is open. Leave arena lives in the Escape/Pause menu. The leaderboard starts expanded, has a translucent background and can be collapsed. Enter-to-Imagine, Home/recovery, preset-build shortcuts and the old Practice/Arena entry choice are superseded. Legacy Practice, room API support and original procedural models remain in source for compatibility and regression checks.
...
Trusted weapon families provide visible sword swings, arrows, automatic tracers/muzzle flashes, and flame emitted from a generated dragon’s authored mouth position. Body and vehicle facing follows movement, never the firing animation. Shots use character yaw, never independent camera orbit. Weapons fire in their neutral forward orientation; camera look never tilts weapons. Fixed mounted weapons shoot along the vehicle’s forward axis. Practice’s touch Jump button invokes jumping directly and never starts an attack; an independently pressed attack can still accompany a jump. Generated held melee weapons swing about local X toward character +Z around the right-hand grip. Anatomical right is local -X: one-handed weapons and punches use that hand; two-handed guns use it as the primary grip; bows use a left grip and right draw. Existing persisted emitter coordinates are translated to the new grip without mirroring creations. Confirmed shot events carry projectile IDs, origins and velocities, so even shots that hit between snapshots have visible travel followed by feedback at the authoritative contact point. Ground strikes now emit impact events, and range expiry does not invent a hit. Immediate local attack animation is separate from confirmed impact events. Confirmed hits flash and flinch the target, change health and play nearby sound. Creation destruction, defeat, blocked hits and 12-second respawn have distinct feedback. Armor, creation health, weapon trade-offs, pickups, KOs and spawn protection remain bounded server rules. Sky drops cycle through health, defense and speed support only. Existing equipment drops are removed from persisted matches. These pickups retain their effects and never replace an equipped or building creation. Weapons and vehicles remain available through creation with no added lifetime. The legacy rover blueprint remains readable for already-created loadouts. Spoken creation still generates new geometry independently. Model-generated code is never executed.

The existing ring road, colored houses and central plaza remain, with four major brick rocks providing additional cover. Physical map objects, including trees, small props, gate posts, targets, crates and plaza pieces, block movement using the same solid geometry as shot cover. Stepped rocks and tree foliage use separate height-aware shapes; real gaps remain open. Low props provide landing and walking support. Large mounts use conservative footprints and need broad routes. Sky rings have a 5.6-meter radius (formerly 4.4); their rims use oriented segments and mount dimensions in the ring plane, leaving the aperture open. Practice scores crossings in either direction through that aperture, once per ring. Builds that do not fit retain the old kit and pose, with a prompt to move into open space and rebuild the saved design. Restoring destroyed cover waits for occupants to leave. Camera obstruction fades houses/rocks locally without moving the camera toward them.

Walking, driving and flying stop immediately when controls release. Flight uses explicit rise/lower and hover. On-foot jumps reach approximately two metres above their starting support. Characters land on solid tops, stand and walk there, and fall naturally after walking off, losing support or dismounting in midair. Head clearance stops upward motion. There is no horizontal momentum, bounce, ramming or scenery crash damage. Ordinary snapshots, equipment changes and connection errors do not intentionally relocate an idle player. Explicit joining and real defeat/respawn can relocate. Acknowledged movement frames prevent duplicate movement on retries and reject pre-death frames after respawn. Expiry retains the displayed pose and offers Rejoin.
...
The trusted Sites identity binds arena sessions to players. The browser cannot set authoritative positions, damage or scores. HTTP synchronization uses acknowledged movement frames, monotonic commands and acknowledged event cursors. Responses send new combat events instead of retransmitting the entire recent event history. Each request owns its I/O; no request waits on another invocation’s pending promise. D1 compare-and-swap and bounded randomized retries protect all competing writes. Join retries prove possession of a random session token and reuse one durable seat, including after a lost response. Rejoining does not wait for a separate leave request. The 12-second request timeout remains, with a clear retry message.
...
This revision integrates GitHub main at `693f7ebb1d947884fece174733ef57e1eadcfcfc` before applying the requested home-screen and Arena-exit changes. `npm run dev` serves Practice and the actual UI. It does not host authenticated Arena services; Arena requests now receive an explicit JSON 503 with a Back to Practice instruction. AI generation requires the existing optional private API preview configuration. The production Worker, authentication and runtime secrets are unchanged.
...
Arena rounds last five minutes from the first join, with one server deadline shared by all clients. Combat stops at the deadline, and the final KO/fall scoreboard appears during a 30-second intermission. A visible countdown replaces manual restart. The next round starts automatically in Arena, resetting scores and restoring health while retaining player sessions, avatars and loadouts. Leaderboards include connected participants in the current round only; departed or timed-out sessions and historic scores are excluded. Separate players with identical names stay separate. Intermission joiners watch the finished round, then enter the next roster. The existing 20-second inactivity timeout applies during play and intermission. Old movement and action packets cannot affect a later round. No weapon or vehicle lifetimes are introduced.

Practice is the renamed solo mode with the same challenges and saved progress. Practice and Arena use one camera rig for equivalent loadouts: framing, orbit and vertical look are shared, and dragging upward can reveal the sky. Camera look stays independent of character-directed firing.
