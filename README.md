...
Trusted weapon families provide visible sword swings, arrows, automatic tracers/muzzle flashes, and flame emitted from a generated dragon’s authored mouth position. Immediate local attack animation is separate from confirmed impact events. Confirmed hits flash and flinch the target, change health and play nearby sound. Creation destruction, defeat, blocked hits and 12-second respawn have distinct feedback. Armor, creation health, weapon trade-offs, pickups, KOs and spawn protection remain bounded server rules. Violet parachutes periodically deliver a Star sword, Brick bow or twin-gun rover, rotating every third supply drop (about 36 seconds). Walk over landed gear empty-handed to equip it; equipment and active assemblies are preserved. The full mount must fit without relocating the player. The rover reuses the captured live design in `validation/live/final-compact-none/armed-car.json`; its geometry ships in the optional supply catalog and needs no new API call. Spoken creation still generates new geometry independently. Model-generated code is never executed.
